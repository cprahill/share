...
// cache: game-v113.js  — four-mode core: trial, tour, explorer, freestyle playground
const BUILD = "113";
...
const PLAY_JUMPS = [
  { t: 0.04, kind: "table", h: 5.4, len: 58 },
...
function seedHazards() {
  ...
  for (let i = 0; i < DIFF.nGround; i++) {
    const t = 0.20 + (i + 0.5) / Math.max(1, DIFF.nGround) * 0.74;
    const at0 = placeOnLat(t, 0);
    const y = terrainH(at0.x, at0.z) + 3.15;
    const g = makeFlyer(i % 3);
    scene.add(g);
    hazards.push({
      t, y, r: 2.05, air: true, kind: "ufo", moving: true, amp: HALF_W * 0.82,
...
function hitHazards(spdAbs, groundY) {
  for (let i = 0; i < hazards.length; i++) {
    ...
    else showToast(h.kind === "ufo" ? "UFO" : "CRASH", 0.4);
...
function seedCoins() {
  ...
  JUMPS.forEach((j, k) => {
    ...
    plantCoin(lip, k % 2 ? -3.2 : 3.2, 3.2);
    plantCoin(air, 0, j.kind === "valley" ? 7.4 : 4.8);
  });
}
...
function spawnStart() {
  ...
  pickups.forEach((p) => {
    const extra = p.pack === "extra";
    const show = isOpenWorld() ? false
      : extra ? (MODE.key === "trial" || isPlayground())
      : true;
...
function addPickup(kind, t, lat, yOff, cool, pack) {
  ...
  mesh.frustumCulled = false;
  scene.add(mesh);
  pickups.push({ kind, x, z, y, mesh, taken: 0, cool: cool || 8, hitR: kind === "RING" ? 5.4 : 3.8, pack: pack || "core" });
}
(function seedPickups() {
  const tanks = [
    [0.10, 4.2], [0.22, -4.0], [0.32, 4.4], [0.42, -3.8],
    [0.54, 4.0], [0.64, -4.2], [0.76, 3.6], [0.88, -4.0]
  ];
  const boosts = [
    [0.16, 0], [0.26, 0], [0.38, 0], [0.48, 0],
    [0.58, 0], [0.70, 0], [0.82, 0], [0.94, 0]
  ];
  tanks.forEach((s) => addPickup("TANK", s[0], s[1], 2.2, 7));
  boosts.forEach((s) => addPickup("BOOST", s[0], s[1], 2.6, 8));
  JUMPS.forEach((j) => {
    if (j.kind !== "valley" && j.kind !== "table") return;
  ...
  const extra = [
    [0.08, 0], [0.14, 4.2], [0.21, -4.2], [0.30, 0], [0.34, 5], [0.41, -5],
    [0.45, 0], [0.52, 4], [0.58, -4], [0.65, 0], [0.72, 4.5], [0.79, -4.5],
    [0.85, 0], [0.91, 4], [0.97, -4]
  ];
  extra.forEach((s, i) => addPickup(i % 2 ? "TANK" : "BOOST", s[0], s[1], i % 2 ? 2.2 : 2.6, 6, "extra"));
  PLAY_JUMPS.forEach((j) => {
    const lip = (j.t + 0.24 * j.len / trackLen) % 1;
    const air = (j.t + 0.40 * j.len / trackLen) % 1;
    addPickup("RING", lip, 0, 5.8, 7, "extra");
    addPickup("RING", air, 0, j.kind === "valley" ? 10 : 7.2, 7, "extra");
  });
})();
