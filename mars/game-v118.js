...
// cache: game-v118.js  — pickup layout: clean start, one hoop per jump
const BUILD = "118";
...
const DIFFS = {
  easy: { key: "easy", tag: "EASY", nGround: 0, nAir: 0, airHz: 0, airSlots: [0.28], slow: 0.64, spin: false, rival: [54, 58, 46], scoreMul: 0 },
  medium: { key: "medium", tag: "MED", nGround: 8, nAir: 0, airHz: 0, airSlots: [0.28], slow: 0.52, spin: false, rival: [60, 64, 50], scoreMul: 2 },
  hard: { key: "hard", tag: "HARD", nGround: 16, nAir: 8, airHz: 1.55, airSlots: [0.26, 0.40], slow: 0.40, spin: true, rival: [68, 78, 48], scoreMul: 5 },
  extra: { key: "extra", tag: "XHARD", nGround: 20, nAir: 12, airHz: 2.7, airSlots: [0.22, 0.34, 0.46], slow: 0.40, spin: true, rival: [74, 86, 46], scoreMul: 8 }
};
...
function parseMode(raw) {
  ...
  if (s === "free" || s === "freestyle" || s === "play") return MODES.free;
...
if (urlParams.get("m")) MODE = parseMode(urlParams.get("m"));
...
const glassMat = new THREE.MeshStandardMaterial({
  color: 0x0c1014, metalness: 0.28, roughness: 0.22, transparent: true, opacity: 0.9,
  envMapIntensity: 0.35, fog: false, emissive: 0x000000, emissiveIntensity: 0
});
const sideGlassMat = new THREE.MeshStandardMaterial({
  color: 0x08090b, metalness: 0.12, roughness: 0.32, transparent: true, opacity: 0.97,
  envMapIntensity: 0.12, fog: false, side: THREE.DoubleSide, depthWrite: true
});
...
function trackH(x, z) {
  ...
  let jy = near.jy;
  if (isPlayground() && near.d < HALF_W + 6) {
...
function wedgeTruck() {
  ...
  const sideL = part(new THREE.BoxGeometry(0.05, 0.58, 1.28), sideGlassMat, -1.14, 1.52, 0.22);
  const sideR = part(new THREE.BoxGeometry(0.05, 0.58, 1.28), sideGlassMat, 1.14, 1.52, 0.22);
  sideL.name = "sideGlassL";
  sideR.name = "sideGlassR";
  sideL.renderOrder = 5;
  sideR.renderOrder = 5;
...
let hurtSafeT = 0;
const HURT_SAFE = 3;
function hurtSafe() { return hurtSafeT > 0; }
...
function stepHurtSafe(dt) {
  if (hurtSafeT > 0) {
    hurtSafeT -= dt;
    truck.visible = Math.floor(hurtSafeT * 14) % 2 === 0;
    if (hurtSafeT <= 0) { hurtSafeT = 0; truck.visible = true; }
  } else if (truck && truck.visible === false) truck.visible = true;
}
function spinOut(t) {
  if (hurtSafe()) return;
...
function showBoot() {
  ...
  if (elBootSub) {
    elBootSub.textContent = MODE.key === "trial"
      ...
      : MODE.key === "free"
      ? "Jump playground. Big air, extra kickers, more boosts. No clock."
  ...
  document.querySelectorAll(".boot-btn").forEach((b) => {
    ...
    } else if (MODE.key === "free") {
      span.textContent = k === "easy" ? "flow" : k === "hard" ? "big tables" : k === "extra" ? "max air" : "kickers";
    } else {
      span.textContent = k === "easy" ? "no obstacles" : k === "hard" ? "jump air · UFOs" : k === "extra" ? "3-layer air · gold paint" : "UFOs · faster rival";
    }
  });
...
function seedHazards() {
  ...
  const flyHz = DIFF.airHz || 1.2;
  for (let i = 0; i < DIFF.nGround; i++) {
    const t = snapOffBigAir(0.20 + (i + 0.5) / Math.max(1, DIFF.nGround) * 0.74);
    const at0 = placeOnLat(t, 0);
    const y = terrainH(at0.x, at0.z) + 2.15;
    const g = makeFlyer(i % 3);
    scene.add(g);
    hazards.push({
      t, y, r: 2.55, air: true, kind: "ufo", moving: true, amp: HALF_W * 0.82,
      phase: i * 1.07, speed: flyHz, mesh: g, coolUntil: 0, x: 0, z: 0
    });
  }
  const jumpAir = JUMPS.filter((j) => j.kind === "valley" || j.kind === "table");
  const slots = DIFF.airSlots || [0.28, 0.40];
  for (let i = 0; i < DIFF.nAir; i++) {
    const j = jumpAir[Math.floor(i / slots.length) % jumpAir.length];
    const u = slots[i % slots.length];
    const t = (j.t + u * j.len / trackLen) % 1;
    const y = terrainH(placeOnLat(t, 0).x, placeOnLat(t, 0).z) + 2.7;
    const g = new THREE.Group();
    ...
    scene.add(g);
    hazards.push({
      t, y, r: 1.65, air: true, kind: "orb", moving: true, amp: HALF_W * 0.72,
...
function stepHazards(dt) {
  for (let i = 0; i < hazards.length; i++) {
    ...
    h.x = at.x; h.z = at.z;
    if (h.mesh) {
      const bob = h.kind === "ufo" ? Math.sin(h.phase * 2.1) * 0.38 : 0;
      h.mesh.position.set(at.x, h.y + bob, at.z);
      h.mesh.rotation.y += dt * (h.kind === "ufo" ? 1.4 : 2.2);
    }
  }
}
function hitHazards(spdAbs, groundY) {
  ...
  for (let i = 0; i < hazards.length; i++) {
    ...
    const hitR = (h.r || 2) + 2.2;
    if (d > hitR) continue;
    const carBot = car.y - 0.15;
    const carTop = car.y + 2.55;
    const pad = h.kind === "ufo" ? 1.9 : 1.45;
    const hBot = hy - pad;
    const hTop = hy + pad;
    ...
    car.speed *= Math.min(DIFF.slow, 0.48);
    if (car.air) car.vy *= 0.32;
    h.phase += 0.85;
    emitDust(car.x, car.y + 0.4, car.z, nx, nz, 12);
    if (spdAbs > 10 && car.spinT <= 0) spinOut(h.kind === "ufo" ? 0.7 : 0.5);
    else {
      showToast(h.kind === "ufo" ? "UFO" : "CRASH", 0.45);
      armHurtSafe();
    }
    return;
  }
...
function seedCoins() {
  ...
  if (MODE.key === "trial" || MODE.key === "hunt" || MODE.key === "free") return;
  for (let i = 0; i < COIN_N; i++) {
    const t = 0.045 + (i + 0.5) / COIN_N * 0.90;
    const lane = i % 6;
  ...
  JUMPS.forEach((j, k) => {
    ...
    const air = (j.t + 0.42 * j.len / trackLen) % 1;
    plantCoin(lip, k % 2 ? -3.2 : 3.2, 1.4);
    plantCoin(air, 0, j.kind === "valley" ? 3.0 : 2.2);
  });
}
function grabCoin(c, who) {
  ...
  if (who === "cpu") {
    ...
    coinsGot += 1;
    if (MODE.key !== "tour" && coinsGot >= coins.length && coins.length) showToast("SWEEP  +" + SWEEP_BONUS, 1.1);
...
function stepCoins(dt) {
  for (let i = 0; i < coins.length; i++) {
    ...
    if (xz <= 5.2) {
      ...
      if (car.y + 2.4 >= cy - 3.2 && car.y - 1.2 <= cy + 8.5) grabCoin(c, "player");
    }
...
const HUNT_CATALOG = [
  { name: "COW", url: "./lib/hunt/cow.glb", h: 2.8, metal: 0.02 },
  { name: "CHICKEN", url: "./lib/hunt/chicken.glb", h: 1.6, metal: 0.02 },
  { name: "SHEEP", url: "./lib/hunt/sheep.glb", h: 1.9, metal: 0.02 },
  { name: "RAPTOR", kind: "engine", h: 4.4, metal: 0.82 },
  { name: "ALIEN", url: "./lib/hunt/alien.glb", h: 2.4, metal: 0.55 },
  { name: "BANANA", url: "./lib/hunt/banana.glb", h: 1.8, metal: 0, hex: 0xFFE135 },
  { name: "DONUT", url: "./lib/hunt/donut-sprinkles.glb", h: 1.6, metal: 0, hex: 0xFF8AB0 },
  { name: "PIZZA", url: "./lib/hunt/pizza.glb", h: 1.9, metal: 0, hex: 0xFFC14D },
  { name: "HOTDOG", url: "./lib/hunt/hot-dog.glb", h: 1.6, metal: 0, hex: 0xE07040 },
  { name: "SNOWMAN", url: "./lib/hunt/snowman.glb", h: 3.0, metal: 0.04 },
  { name: "GINGER", url: "./lib/hunt/gingerbread-man.glb", h: 2.1, metal: 0, hex: 0xC4783A },
  { name: "PUMPKIN", url: "./lib/hunt/pumpkin-carved.glb", h: 1.9, metal: 0, hex: 0xFF7A22 },
  { name: "PALM", url: "./lib/hunt/palm.glb", h: 5.2, metal: 0.06 },
  { name: "CACTUS", url: "./lib/hunt/cactus.glb", h: 2.8, metal: 0.06, hex: 0x3DBF7A }
];
function huntNeed() {
...
const HUNT_SITES = [
  { t: 0.05, side: 1, dist: 148 },
  { t: 0.13, side: -1, dist: 162 },
  { t: 0.19, side: 1, dist: 176 },
  { t: 0.25, side: -1, dist: 154 },
  { t: 0.32, side: 1, dist: 168 },
  { t: 0.39, side: -1, dist: 180 },
  { t: 0.46, side: 1, dist: 158 },
  { t: 0.53, side: -1, dist: 172 },
  { t: 0.60, side: 1, dist: 164 },
  { t: 0.68, side: -1, dist: 178 },
  { t: 0.75, side: 1, dist: 152 },
  { t: 0.82, side: -1, dist: 166 },
  { t: 0.90, side: 1, dist: 174 },
  { t: 0.97, side: -1, dist: 156 }
];
function huntBlocked(x, z, r) {
  if (nearShipFire(x, z)) return true;
  if (minDistToPath(x, z) < HALF_W + 20) return true;
  for (let i = 0; i < colliders.length; i++) {
    const c = colliders[i];
    if (Math.hypot(x - c.x, z - c.z) < (c.r || 2) + r + 4) return true;
  }
  return false;
}
function huntSiteAt(i) {
  const s = HUNT_SITES[i % HUNT_SITES.length];
  const p = alongTrack(s.t, s.side, s.dist);
  const rgt = p.right;
  for (let k = 0; k < 10; k++) {
    const x = p.x + rgt.x * s.side * k * 12;
    const z = p.z + rgt.z * s.side * k * 12;
    if (!huntBlocked(x, z, 6)) return { x, z, yaw: p.yaw };
  }
...
function dressHuntMesh(root, spec) {
  ...
  root.traverse((o) => {
    ...
    mats.forEach((m) => {
      ...
      const metal = spec.metal != null ? spec.metal : 0.08;
      if (m.metalnessMap == null) m.metalness = metal;
      if (m.roughness == null || m.roughness > 0.82) m.roughness = 0.48;
      if (!m.map && !hasColor && spec.hex && m.color) m.color.setHex(spec.hex);
      if (m.emissive) {
        if (spec.hex) m.emissive.setHex(spec.hex);
        else if (m.color) m.emissive.copy(m.color);
        m.emissiveIntensity = spec.hex ? 0.22 : 0.12;
      }
      m.needsUpdate = true;
    });
  });
...
function sitHuntItem(root, x, z, worldH, yaw) {
  ...
  root.updateMatrixWorld(true);
  box.setFromObject(root);
  root.position.y = groundH(x, z) - box.min.y - 0.02;
  root.traverse((o) => {
    if (!o.isMesh) return;
    o.castShadow = true;
    o.receiveShadow = true;
...
function seedHunt() {
  ...
  HUNT_CATALOG.forEach((spec, i) => {
    if (MODE.key === "hunt" && i >= n) return;
    const p = huntSiteAt(i);
...
function renderHuntList() {
  ...
  if (!el) return;
  if (!isOpenWorld()) { el.innerHTML = ""; return; }
  el.innerHTML = huntItems.map((it) => {
    return "<li class=\"" + (it.found ? "got" : "") + "\">" + it.name + "</li>";
  }).join("");
}
let snapCool = 0;
...
function raceScore() {
  ...
  if (MODE.key === "free") return huntFound * 400;
  if (MODE.key === "tour") {
...
const BOARD_KEY = "rdb-board-v1";
function emptyBoard() { return { raid: [], trial: [], tour: [], hunt: [] }; }
function loadBoard() {
  try {
    ...
    if (raw && typeof raw === "object") {
      return {
        ...
        trial: Array.isArray(raw.trial) ? raw.trial : [],
        tour: Array.isArray(raw.tour) ? raw.tour : [],
        hunt: Array.isArray(raw.hunt) ? raw.hunt : []
      };
    }
  } catch (err) {}
  const b = emptyBoard();
  try {
    ...
    if (Array.isArray(old) && old.length) b.raid = old;
  } catch (err2) {}
  return b;
}
...
function listBoard(mode) {
  return sortBoard(mode, loadBoard()[mode] || []);
}
function addBoardRow(mode, row) {
  const b = loadBoard();
  const list = (b[mode] || []).concat(row);
  b[mode] = sortBoard(mode, list);
  saveBoard(b);
  return b[mode];
}
function rowHtml(mode, r) {
  if (mode === "trial") {
    return "<li><b>" + (r.name || "----") + "</b>  " + fmtTime(r.time || 0) + "  <span>" + (r.diff || "") + "</span></li>";
  }
  return "<li><b>" + (r.name || "----") + "</b>  " + r.score + "  <span>" + (r.diff || "") + " · " + (r.coins || 0) + "c · " + fmtTime(r.time || 0) + "</span></li>";
}
function renderHsBoard() {
  ...
  ["raid", "trial", "tour", "hunt"].forEach((m) => {
...
function submitHs() {
  ...
  const row = {
    ...
    cup: cupTier(), mode: MODE.key, ghost: encGhost(ghostRec) || ""
  };
  addBoardRow(MODE.key, row);
  hsSubmitted = true;
  renderHsBoard();
...
document.querySelectorAll("[data-home]").forEach((btn) => {
  btn.addEventListener("click", (e) => {
    ...
    else if (k === "free") { MODE = MODES.free; showBoot(); }
...
function spawnStart() {
  ...
  pickups.forEach((p) => {
    const show = isOpenWorld() ? false
      : p.pack === "play" ? isPlayground()
      : p.pack === "extra" ? (MODE.key === "trial" || isPlayground())
      : true;
...
function drive(dt) {
  ...
  const accel = car.air ? (isPlayground() ? 18 : 14) : 48;
  const punch = car.turboT > 0 ? 2.85 : (boosting ? 2.35 : 1);
  ...
  const hit = jumpAtT(proj.t) || (isPlayground() ? jumpAtT(proj.t, PLAY_JUMPS) : null);
  ...
  if (!car.air) {
    ...
    if (car.spinT <= 0 && spdAbs > 18 && onTake && !isOpenWorld()) {
      ...
      if (isPlayground()) { car.vy += 5.4; car.speed += 8; }
    }
  } else {
    car.vy -= (isPlayground() ? 21 : GRAVITY) * dt;
    car.y += car.vy * dt;
...
function paintRivalKit(kit, hex) {
  ...
  kit.traverse((o) => {
    if (!o.isMesh) return;
    if (o.name === "windshield" || o.name === "sideGlassL" || o.name === "sideGlassR") o.material = glass;
...
function addPickup(kind, t, lat, yOff, cool, pack) {
  ...
  mesh.frustumCulled = false;
  scene.add(mesh);
  pickups.push({ kind, x, z, y, mesh, taken: 0, cool: cool || 8, hitR: kind === "RING" ? 3.4 : 3.8, pack: pack || "core" });
}
// start t<0.18 empty. TANK/BOOST on dirt. one RING at each valley/table apex.
// pack extra = Time Trial + Freestyle. pack play = Freestyle kickers only.
(function seedPickups() {
  const tanks = [
    [0.19, 4.4], [0.31, -4.4], [0.45, 4.4], [0.54, -4.4],
    [0.63, 4.4], [0.75, -4.4], [0.83, 4.4], [0.94, -4.4]
  ];
  const boosts = [
    [0.21, -4.0], [0.29, 4.0], [0.38, -4.0], [0.49, 4.0],
    [0.58, -4.0], [0.71, 4.0], [0.81, -4.0], [0.92, 4.0]
  ];
  tanks.forEach((s) => addPickup("TANK", snapOffBigAir(s[0]), s[1], 2.2, 7));
  boosts.forEach((s) => addPickup("BOOST", snapOffBigAir(s[0]), s[1], 2.5, 8));
  JUMPS.forEach((j) => {
    if (j.kind !== "valley" && j.kind !== "table") return;
    const apex = (j.t + 0.38 * j.len / trackLen) % 1;
    addPickup("RING", apex, 0, j.kind === "valley" ? 2.4 : 2.15, 8);
  });
  const extra = [
    [0.24, 5.4], [0.35, -5.4], [0.52, 5.4], [0.64, -5.4],
    [0.77, 5.4], [0.90, -5.4]
  ];
  extra.forEach((s, i) => addPickup(i % 2 ? "TANK" : "BOOST", snapOffBigAir(s[0]), s[1], i % 2 ? 2.2 : 2.5, 6, "extra"));
  PLAY_JUMPS.forEach((j) => {
    if (j.t < 0.16) return;
    const apex = (j.t + 0.36 * j.len / trackLen) % 1;
    addPickup("RING", apex, 0, 2.3, 7, "play");
  });
})();
...
function plantPlayRamps() {
  ...
  const mat = new THREE.MeshStandardMaterial({ color: 0xC8B090, roughness: 0.82, metalness: 0.08 });
  PLAY_JUMPS.forEach((j) => {
    const u = (j.t + 0.32 * j.len / trackLen) % 1;
    const at = placeOnLat(u, 0);
    const g = new THREE.Mesh(new THREE.BoxGeometry(HALF_W * 1.7, Math.max(1.6, j.h * 0.42), j.len * 0.42), mat);
    g.position.set(at.x, groundH(at.x, at.z) + Math.max(0.9, j.h * 0.22), at.z);
    g.rotation.y = at.yaw;
    g.castShadow = true;
...
function endCeremony() {
  ...
  if (ceremony.spray) ceremony.spray.visible = false;
  if (scene.fog) scene.fog.color.setHex(FOG_PEACH);
  document.body.classList.remove("ceremony-hide-hud");
}
...
function hudBind() {
  ...
  if (MODE.key === "hunt") {
    ...
    elGateDist.textContent = nxt
      ? (nxt.name + "  " + Math.round(Math.hypot(nxt.x - car.x, nxt.z - car.z)) + " m")
      : "DONE";
    if (elCoinLab) elCoinLab.textContent = "SHOTS";
  } else if (isPlayground()) {
    elGateIdx.textContent = car.air ? "AIR" : "PLAY";
    elGateDist.textContent = Math.round(Math.abs(car.speed) * 2.236936) + " MPH";
    if (elCoinLab) elCoinLab.textContent = "AIR";
  } else {
  ...
  if (elCoinVal) {
    ...
    else if (isPlayground()) elCoinVal.textContent = car.air ? "UP" : "—";
...
function loop() {
  ...
  if (!finished && !startLock && MODE.key !== "free") timeMs += dt * 1000;
