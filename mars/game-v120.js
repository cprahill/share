...
// cache: game-v120.js  — freestyle: open course or track playground
const BUILD = "120";
...
let freeKind = "track";
function isOpenWorld() { return MODE.key === "hunt" || (MODE.key === "free" && freeKind === "open"); }
function isPlayground() { return MODE.key === "free" && freeKind === "track"; }
...
(function marsLand() {
  ...
  function plantDune(x, z) {
    ...
    dummy.updateMatrix();
    duneSpots.push({ x, z, r: Math.max(dummy.scale.x, dummy.scale.z) * 0.58 });
...
function showBoot() {
  ...
  if (elBootSub) {
    elBootSub.textContent = MODE.key === "trial"
      ...
      : MODE.key === "free"
      ? "Open course to roam the colony, or Track for kickers, boosts, and big air. No clock."
...
const HUNT_CATALOG = [
  ...
  { name: "PIZZA", url: "./lib/hunt/pizza.glb", h: 1.9, hex: 0xFFC14D },
  { name: "HOTDOG", url: "./lib/hunt/hot-dog.glb", h: 1.6, hex: 0xE07040 },
  { name: "SNOWMAN", url: "./lib/hunt/snowman.glb", h: 3.0, hex: 0xF4F7FA },
  ...
  { name: "PALM", url: "./lib/hunt/palm.glb", h: 5.2, hex: 0x3DAA5A },
...
function huntSiteAt(i) {
  ...
  for (let k = 0; k < 14; k++) {
    const along = ((k % 3) - 1) * 9;
    const out = (k % 2 ? 1 : -1) * (k * 4);
    const x = p.x + rgt.x * s.side * out + tan.x * along;
    const z = p.z + rgt.z * s.side * out + tan.z * along;
    if (!huntBlocked(x, z, 5.5)) return { x, z, yaw: p.yaw };
  }
  const q = alongTrack(s.t, s.side, 40);
...
function seedHunt() {
  ...
  HUNT_CATALOG.forEach((spec, i) => {
    ...
    loadGlb(spec.url, (root) => {
      if (nearShipFire(p.x, p.z) || huntBlocked(p.x, p.z, 5)) {
        const q = huntSiteAt(i + 3);
...
function spawnStart() {
  ...
  pickups.forEach((p) => {
    const show = MODE.key === "hunt" ? false
...
function hudBind() {
  ...
  if (MODE.key === "hunt") {
    ...
    elGateIdx.textContent = "ROAM";
    elGateDist.textContent = "";
    if (elCoinLab) elCoinLab.textContent = "FIND";
  } else {
