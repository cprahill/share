...
// cache: game-v124.js  — explorer easy: one item per site, no stacked pair
const BUILD = "124";
...
(function marsLand() {
  ...
  function plantDune(x, z, i, t) {
    if (n >= 56) return;
    // Big red ovals: keep uses footprint, not center. Start cluster was on the launch dirt.
    if (t < 0.18 || t > 0.97) return;
    const s = 20 + Math.random() * 18;
    const sy = s * 0.20;
    dummy.scale.set(s * (1.8 + Math.random() * 0.6), sy, s * (1.1 + Math.random() * 0.5));
    const rad = Math.max(dummy.scale.x, dummy.scale.z) * 0.55;
    const pr = projectTrack(new THREE.Vector3(x, 0, z));
    const sign = pr.offset >= 0 ? 1 : -1;
    for (let k = 0; k < 14; k++) {
      if (minDistToPath(x, z) >= HALF_W + 8 + rad) break;
      x += pr.right.x * sign * 14;
      z += pr.right.z * sign * 14;
    }
    if (minDistToPath(x, z) < HALF_W + 8 + rad) return;
  ...
  clusterAlong(0.20, 0.34, -1, 140, 240, IS_MOBILE ? 5 : 10, plantDune);
  clusterAlong(0.30, 0.46, -1, 150, 250, IS_MOBILE ? 5 : 10, plantDune);
  clusterAlong(0.58, 0.70, 1, 140, 240, IS_MOBILE ? 5 : 9, plantDune);
  clusterAlong(0.72, 0.86, 1, 145, 245, IS_MOBILE ? 4 : 9, plantDune);
  clusterAlong(0.88, 0.94, -1, 140, 220, IS_MOBILE ? 3 : 6, plantDune);
  dunes.count = n;
...
function huntSiteAt(i, skip) {
  ...
  for (let s = 0; s < n; s++) {
    ...
    const p = alongTrack(site.t, site.side, site.dist);
  ...
  const q = alongTrack(site.t, site.side, 80);
