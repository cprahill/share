...
// cache: game-v126.js  — fullscreen + visualViewport fit (mobile + desktop)
const BUILD = "126";
...
function fitView() {
  ...
  const on = isFs() || document.documentElement.classList.contains("fs-fit");
  if (fs) {
  ...
  if (hf) hf.textContent = on ? "EXIT FULL" : "FULL SCREEN";
}
async function toggleFullscreen(e) {
  ...
  if (root.classList.contains("fs-fit")) {
    root.classList.remove("fs-fit");
    fitView();
    return;
  }
...
}
{
  ...
  addEventListener("orientationchange", () => setTimeout(fitView, 80));
...
(function marsLand() {
  ...
  function duneHitsDirt(x, z, rad) {
    if (minDistToPath(x, z) < HALF_W + 8 + rad) return true;
    for (let i = 0; i < 8; i++) {
      const a = i * Math.PI * 0.25;
      if (minDistToPath(x + Math.cos(a) * rad, z + Math.sin(a) * rad) < HALF_W + 4) return true;
    }
    return false;
  }
