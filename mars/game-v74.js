...
// cache: game-v74.js  — open desert, no walls, no starship cloud, real jumps
const BUILD = "74";
const GATE_COUNT = 5;
...
const elPadFire = document.getElementById("pad-fire");
const elPadMine = document.getElementById("pad-mine");
...
function padRoleFromTarget(t) {
  ...
  if (t.closest("#pad-fire")) return "fire";
  if (t.closest("#pad-mine")) return "mine";
...
function onPtrDown(e) {
  ...
  if (hit === "fire") { fireItem(); setPadHeld(elPadFire, true); if (e.cancelable) e.preventDefault(); return; }
  if (hit === "mine") { dropMine(); setPadHeld(elPadMine, true); if (e.cancelable) e.preventDefault(); return; }
  let role = hit;
...
function onPtrUp(e) {
  ...
  setPadHeld(elPadFire, false);
  setPadHeld(elPadMine, false);
  touchRecompute();
}
...
scene.fog = new THREE.FogExp2(FOG_PEACH, 0.0018);
...
for (let i = 0; i <= 22; i++) {
  const u = i / 22;
  const z = u * 1700;
  const x = 24 * Math.sin(u * Math.PI * 2.15);
...
for (let i = 21; i >= 0; i--) {
  const u = i / 22;
  const z = u * 1700;
  const x = 460 + 24 * Math.sin(u * Math.PI * 2.15);
  pathPts.push(new THREE.Vector3(x, 0, z));
}
...
const JUMPS = [
  { t: 0.07, kind: "kicker", h: 9, len: 38, n: 0 },
  { t: 0.16, kind: "whoops", h: 2.4, len: 110, n: 8 },
  { t: 0.26, kind: "table", h: 8, len: 58, n: 0 },
  { t: 0.38, kind: "kicker", h: 15, len: 42, n: 0 },
  { t: 0.50, kind: "whoops", h: 2.2, len: 90, n: 7 },
  { t: 0.62, kind: "table", h: 10, len: 66, n: 0 },
  { t: 0.74, kind: "kicker", h: 17, len: 48, n: 0 },
  { t: 0.88, kind: "step", h: 9, len: 110, n: 0 }
];
...
function jumpProfile(j, u) {
  ...
  if (j.kind === "whoops") {
    ...
    return h * win * (0.45 + 0.55 * Math.sin(u * j.n * Math.PI));
  }
  if (j.kind === "kicker") {
    if (u < 0.72) return h * Math.pow(u / 0.72, 1.2);
    return h * Math.max(0, 1 - (u - 0.72) / 0.28) * 0.12;
  }
  if (j.kind === "table") {
    if (u < 0.18) return h * (u / 0.18);
    if (u < 0.62) return h;
    return h * Math.max(0, 1 - (u - 0.62) / 0.38);
  }
  if (u < 0.34) return h * (u / 0.34);
  if (u < 0.72) return h;
  return h * (0.2 + 0.8 * Math.max(0, 1 - (u - 0.72) / 0.28));
}
function jumpYAtT(t) {
  const s = t * trackLen;
  let y = 0;
  for (let i = 0; i < JUMPS.length; i++) {
    const j = JUMPS[i];
    const start = j.t * trackLen - j.len * 0.25;
    const u = (s - start) / j.len;
    if (u < 0 || u > 1) continue;
    y += jumpProfile(j, u);
  }
  return y;
...
function terrainH(x, z) {
  const base = Math.sin(x * 0.02) * Math.cos(z * 0.016) * 0.45 + Math.sin(x * 0.11 + z * 0.09) * 0.12;
  const hills = Math.sin(x * 0.007 + 1.7) * Math.cos(z * 0.0055) * 1.65;
  const near = nearPathSample(x, z);
  const fade = 1 - smoothstep(HALF_W + 1, HALF_W + 10, near.d);
  return base + hills * (1 - fade * 0.75) + near.jy * fade;
}
...
const ground = new THREE.Mesh(
  ...
  new THREE.MeshStandardMaterial({ map: DIRT_GROUND, color: 0xC47858, roughness: 0.96, metalness: 0.02 })
);
ground.rotation.x = -Math.PI / 2;
ground.receiveShadow = true;
{
  const gpos = ground.geometry.attributes.position;
  for (let i = 0; i < gpos.count; i++) gpos.setZ(i, terrainH(gpos.getX(i), gpos.getY(i)));
...
for (let i = 0; i <= ribbonN; i++) {
  ...
  ribbonPos.push(p.x - r.x * (HALF_W - 0.3), h, p.z - r.z * (HALF_W - 0.3));
  ribbonPos.push(p.x + r.x * (HALF_W - 0.3), h, p.z + r.z * (HALF_W - 0.3));
  ribbonUv.push(t * 40, 0, t * 40, 1);
}
for (let i = 0; i < ribbonN; i++) {
  const i0 = i * 2;
...
function addEdge(offA, offB) {
  ...
  for (let i = 0; i <= n; i++) {
    ...
    const tan = curve.getTangentAt(t);
    const r = new THREE.Vector3(tan.z, 0, -tan.x).normalize();
    const h = terrainH(p.x, p.z) + RIBBON_LIFT + 0.12;
...
addEdge(-(HALF_W - 0.05), -(HALF_W - 0.55));
addEdge((HALF_W - 0.55), (HALF_W - 0.05));

(function markJumps() {
  JUMPS.forEach((j) => {
    const uLip = j.kind === "kicker" ? 0.72 : j.kind === "table" ? 0.18 : 0.34;
    const s = j.t * trackLen - j.len * 0.25 + j.len * uLip;
    const t = ((s / trackLen) % 1 + 1) % 1;
    const p = curve.getPointAt(t);
    const tan = curve.getTangentAt(t);
    const y = terrainH(p.x, p.z) + 0.22;
    const stripe = new THREE.Mesh(
      new THREE.BoxGeometry(HALF_W * 2 - 0.6, 0.18, 1.1),
      new THREE.MeshBasicMaterial({ color: j.kind === "kicker" ? 0xFFEE55 : 0xE8B923, fog: false })
    );
...
(function addJumpRamps() {
  const mat = new THREE.MeshStandardMaterial({
    map: DIRT_TRACK, color: 0x8A4A32, roughness: 0.97, metalness: 0.02, flatShading: true
  });
  JUMPS.forEach((j) => {
    if (j.kind === "whoops") return;
    const n = Math.max(16, (j.len / 2) | 0);
    const pos = [], idx = [];
    const start = j.t * trackLen - j.len * 0.25;
    for (let i = 0; i <= n; i++) {
      const u = i / n;
      const t = (((start + u * j.len) / trackLen) % 1 + 1) % 1;
      const p = curve.getPointAt(t);
      const tan = curve.getTangentAt(t);
      const r = new THREE.Vector3(tan.z, 0, -tan.x).normalize();
      const yTop = terrainH(p.x, p.z);
      const rise = jumpProfile(j, u);
      const yBot = yTop - rise - 0.5;
      const hw = HALF_W + 0.4;
      pos.push(
        p.x - r.x * hw, yBot, p.z - r.z * hw,
        p.x - r.x * hw, yTop, p.z - r.z * hw,
        p.x + r.x * hw, yTop, p.z + r.z * hw,
        p.x + r.x * hw, yBot, p.z + r.z * hw
      );
    }
...
const sky = new THREE.Mesh(
  ...
  new THREE.ShaderMaterial({
    ...
    fragmentShader: "varying vec3 w; void main(){ float h = normalize(w).y; vec3 hor = vec3(0.92,0.68,0.52); vec3 zen = vec3(0.55,0.42,0.38); gl_FragColor = vec4(mix(hor, zen, max(h,0.0)),1.0); }"
  })
);
scene.add(sky);
...
(function marsLand() {
  ...
  function scatter(max, minClear, place) {
    let n = 0;
    for (let k = 0; k < max * 4 && n < max; k++) {
      const t = Math.random();
      const p = curve.getPointAt(t);
      const tan = curve.getTangentAt(t);
      const right = new THREE.Vector3(tan.z, 0, -tan.x).normalize();
      const side = Math.random() < 0.5 ? 1 : -1;
      const dist = HALF_W + minClear + Math.random() * 90;
      const x = p.x + right.x * side * dist;
      const z = p.z + right.z * side * dist;
      if (minDistToPath(x, z) < HALF_W + minClear) continue;
      place(x, z, n);
      n++;
    }
    return n;
  ...
  const dunes = new THREE.InstancedMesh(duneGeo, duneMat, 110);
  dunes.castShadow = true;
  dunes.receiveShadow = true;
  dunes.count = scatter(70, 90, (x, z, i) => {
    const s = 4 + Math.random() * 11;
    dummy.position.set(x, terrainH(x, z) - s * 0.35, z);
    dummy.scale.set(s * (0.8 + Math.random()), s * 0.32, s * (0.8 + Math.random()));
  ...
  for (let i = 0; i < 16; i++) {
    ...
    const right = new THREE.Vector3(tan.z, 0, -tan.x).normalize();
    const side = i % 2 ? 1 : -1;
    const dist = HALF_W + 30 + (i % 4) * 12;
    const x = p.x + right.x * side * dist;
    const z = p.z + right.z * side * dist;
    if (minDistToPath(x, z) < HALF_W + 18) continue;
    const r = 6 + (i % 3) * 3;
    const crater = new THREE.Mesh(
      new THREE.TorusGeometry(r, 1.1, 6, 16),
      new THREE.MeshStandardMaterial({ color: 0x6A3224, roughness: 0.96, metalness: 0.02 })
    );
  ...
  scatter(10, 18, (x, z) => {
    ...
    crate.rotation.y = Math.random() * 6;
  ...
  scatter(6, 22, (x, z) => {
    const panel = new THREE.Mesh(new THREE.BoxGeometry(3.4, 0.06, 1.6), panelMat);
    panel.position.set(x, terrainH(x, z) + 0.7, z);
    panel.rotation.set(-0.35, Math.random() * 6, 0);
...
(function scatterRocks() {
  ...
  urls.forEach((url, vi) => {
    loader.load(url, (geo) => {
      ...
      for (let k = 0; k < 8; k++) {
        const spot = placeOutside(14 + k * 3);
        if (!spot) continue;
        let s = (2.4 + (k % 3) * 0.5) / Math.max(h, 0.2);
        const foot = span * s;
        if (minDistToPath(spot.x, spot.z) - foot * 0.5 < CLEAR) continue;
        const m = new THREE.Mesh(geo, mat);
        m.scale.setScalar(s);
        m.position.set(spot.x, terrainH(spot.x, spot.z) - 0.1, spot.z);
        m.rotation.y = k * 1.7 + vi;
        m.castShadow = true;
        m.receiveShadow = true;
        scene.add(m);
        addCollider(spot.x, spot.z, Math.max(1.6, foot * 0.38));
      }
    });
  });
})();
...
(function scatterHills() {
  ...
  urls.forEach((url, vi) => {
    loader.load(url, (geo) => {
      ...
      JUMPS.forEach((j, ji) => {
        if (vi === 2 && j.kind === "whoops") return;
        const p = curve.getPointAt(j.t);
        const tan = curve.getTangentAt(j.t);
        const right = new THREE.Vector3(tan.z, 0, -tan.x).normalize();
        const side = vi === 1 ? 1 : -1;
        const dist = HALF_W + 90 + (vi * 14);
        const x = p.x + right.x * side * dist;
        const z = p.z + right.z * side * dist;
        const s = (8 + vi * 3) / h;
        const foot = span * s;
        if (minDistToPath(x, z) - foot * 0.45 < HALF_W + 14) return;
        const m = new THREE.Mesh(geo, mat);
        m.scale.setScalar(s);
...
(function placeMountains() {
  const loader = new STLLoader();
  const spots = [
    { url: "./mesh/mountain-a.stl", x: -160, z: 420 },
    { url: "./mesh/mountain-b.stl", x: -180, z: 1100 },
    { url: "./mesh/mountain-c.stl", x: 780, z: 800 },
    { url: "./mesh/mountain-a.stl", x: -280, z: 700 },
    { url: "./mesh/mountain-b.stl", x: 900, z: 1400 },
    { url: "./mesh/mountain-c.stl", x: 850, z: 300 }
  ];
  spots.forEach((sp) => {
...
const rivalKitGeo = { body: null };
{
  ...
  loader.load("./mesh/worker.stl", (geo) => {
    ...
    for (let i = 0; i < 5; i++) {
      const spot = placeOutside(22 + i * 4);
      if (!spot) continue;
      const m = new THREE.Mesh(geo, mat);
      m.position.set(spot.x, terrainH(spot.x, spot.z), spot.z);
      m.rotation.y = i * 1.1;
      m.castShadow = true;
      scene.add(m);
    }
...
const gateTs = [0.12, 0.32, 0.52, 0.72, 0.90];
...
const elKosVal = document.getElementById("kos-val");
const elFinishKos = document.getElementById("finish-kos");
...
const car = {
  ...
  speed: 0, lat: 0, boost: 1, overheating: false, hp: 100, item: "GUN", ammo: 12, spinT: 0,
  vy: 0, air: false
};
const shots = [];
const mines = [];
...
function sitWheels() {
  ...
  truck.userData.wheels.forEach((w) => {
    ...
    w.hub.position.set(w.x, THREE.MathUtils.clamp(_w.y, wr - 0.04, wr + 0.55), w.z);
  });
}
...
function fireItem() {
  if (finished || car.spinT > 0 || fireCd > 0) return;
  const kind = car.item;
  ...
  fireCd = laser ? 0.16 : 0.11;
  muzzle.visible = true;
  muzzle.material.color.setHex(laser ? 0x66F0FF : 0xFFEE88);
  muzzleT = 0.07;
  const sx = Math.sin(car.yaw), cz = Math.cos(car.yaw);
  const mesh = new THREE.Mesh(
    laser ? new THREE.CylinderGeometry(0.06, 0.06, 3.4, 6) : new THREE.SphereGeometry(0.16, 6, 6),
    new THREE.MeshBasicMaterial({ color: laser ? 0x66F0FF : 0xFFB020, toneMapped: false, fog: false })
  );
  if (laser) mesh.rotation.x = Math.PI / 2;
  const spd = laser ? 150 : 110;
  const s = {
    x: car.x + sx * 3.1, y: car.y + 1.1, z: car.z + cz * 3.1,
    vx: sx * spd, vz: cz * spd, life: laser ? 0.85 : 1.1,
    dmg: laser ? 34 : 18, mesh, from: "player"
  };
...
function dropMine() {
  if (finished || car.spinT > 0) return;
  if (car.item !== "MINE" || car.ammo <= 0) return;
  car.ammo -= 1;
  if (car.ammo <= 0) car.item = "NONE";
  const sx = Math.sin(car.yaw), cz = Math.cos(car.yaw);
  const x = car.x - sx * 3.5, z = car.z - cz * 3.5;
  const mesh = new THREE.Mesh(
    new THREE.CylinderGeometry(0.5, 0.58, 0.16, 10),
    new THREE.MeshStandardMaterial({ color: 0x2a2a28, metalness: 0.55, roughness: 0.4, emissive: 0x881100, emissiveIntensity: 0.45 })
  );
  mesh.position.set(x, terrainH(x, z) + 0.1, z);
  scene.add(mesh);
  mines.push({ x, z, mesh, arm: 0.65 });
}
...
function spawnStart() {
  ...
  car.boost = 1; car.overheating = false; car.hp = 100; car.item = "GUN"; car.ammo = 12; car.spinT = 0;
  car.vy = 0; car.air = false;
  fireCd = 0; muzzleT = 0; if (muzzle) muzzle.visible = false;
  kos = 0;
  clearProjectiles();
  pickups.forEach((p) => { p.taken = 0; if (p.mesh) p.mesh.visible = true; });
  rivals.forEach((r, i) => {
    r.spinT = 0; r.hp = 80; r.downT = 0;
    r.t = START_T + 0.07 + i * 0.04;
    r.speed = 78 + i * 6;
    if (r.mesh) r.mesh.rotation.z = 0;
  });
...
function drive(dt) {
  ...
  const accel = car.air ? 16 : 68;
  if (th > 0) car.speed += th * accel * (boosting ? 2.35 : 1) * (1 - Math.max(0, car.speed) / (vmax + 18)) * dt;
  else if (th < 0) car.speed += th * (car.speed > 0.4 ? 48 : 16) * dt;
  if (!car.air) car.speed -= car.speed * (0.09 + (hb ? 1.35 : 0) + spdAbs * 0.0012) * dt;
  if (car.speed > vmax) car.speed = THREE.MathUtils.lerp(car.speed, vmax, 1 - Math.exp(-1.4 * dt));
  ...
  const lookT = Math.max(8, spdAbs * 0.12) / trackLen;
  const jNow = jumpYAtT(proj.t);
  const jFwd = jumpYAtT((proj.t + lookT) % 1);
  if (!car.air) {
    car.y = groundY;
    car.vy = 0;
    const wantPitch = THREE.MathUtils.clamp(Math.atan2(yA - yB, 6.4), -0.62, 0.62);
    car.pitch += (wantPitch - car.pitch) * Math.min(1, dt * 8);
    const crest = jNow > 1.15 && jFwd < jNow - 0.65;
    const lip = (groundY - yA) > 1.4;
    if (car.spinT <= 0 && spdAbs > 8 && (crest || lip)) {
      car.air = true;
      car.vy = Math.min(26, 5 + jNow * 1.15 + spdAbs * 0.08);
      if (boosting) car.vy *= 1.32;
      showToast(boosting ? "BOOST JUMP" : "AIR", 0.4);
    }
  } else {
    car.vy -= GRAVITY * dt;
    car.y += car.vy * dt;
    car.pitch += th * 1.7 * dt;
    car.pitch = THREE.MathUtils.clamp(car.pitch, -0.75, 0.75);
    if (bo && car.boost > 0) car.vy += 10 * dt;
    if (car.y <= groundY) {
      const hard = car.vy < -20 || Math.abs(car.pitch) > 0.64;
      const clean = !hard && Math.abs(car.pitch) < 0.28 && car.vy > -14;
      car.y = groundY;
      car.vy = 0;
      car.air = false;
      if (hard && spdAbs > 10) spinOut(1.05);
      else {
        if (clean) car.speed = Math.min(vmax, car.speed + 10);
    ...
    if (prevGateSide < 0.15 && side >= 0 && Math.abs(lat) < g.halfW + 1.4 && spdAbs > MIN_GATE_SPEED) {
      ...
      if (nextGate === GATE_COUNT - 1) {
        if (gatesHit >= GATE_COUNT && lapDriven >= MIN_LAP_FRAC) {
          ...
          if (elFinishKos) elFinishKos.textContent = kos + " KO";
...
function spawnRivals() {
  if (!rivals.length) {
    for (let i = 0; i < 6; i++) {
      const hex = RIVAL_COLORS[i % RIVAL_COLORS.length];
      const monster = i % 2 === 0;
      rivals.push({
        mesh: makeRival("wedge", hex, monster),
        t: START_T + 0.07 + i * 0.04,
        lat: (i % 2 ? 1 : -1) * (3.2 + (i % 3) * 1.1),
        speed: 78 + i * 6 + Math.random() * 8,
        hp: 80, spinT: 0, downT: 0, fireT: 1.5 + Math.random(),
        hex, monster, kind: "wedge", upgraded: false
      });
    }
  }
  rivals.forEach((r, i) => {
    if (r.upgraded) return;
    const want = i % 3 === 0 ? "truck" : i % 3 === 1 ? "interceptor" : "wedge";
    if (want === "wedge") { r.upgraded = true; return; }
    if (want === "truck" && !playerKitReady()) return;
    if (want === "interceptor" && !rivalKitGeo.body) return;
    scene.remove(r.mesh);
    r.kind = want;
    r.mesh = makeRival(want, r.hex, r.monster);
    r.upgraded = true;
  });
}
function grant(kind) {
  if (kind === "BOOST") { car.boost = 1; car.overheating = false; showToast("BOOST", 0.6); return; }
  if (kind === "HEALTH") { car.hp = Math.min(100, car.hp + 40); showToast("REPAIR", 0.6); return; }
  car.item = kind;
  car.ammo = kind === "GUN" ? 14 : kind === "LASER" ? 8 : 3;
  showToast(kind + " x" + car.ammo, 0.7);
}
(function seedPickups() {
  const kinds = ["BOOST", "HEALTH", "GUN", "LASER", "MINE"];
  const cols = { BOOST: 0xE23B32, HEALTH: 0x3de8ff, GUN: 0xFFB020, LASER: 0x66F0FF, MINE: 0xC0C0C0 };
  for (let i = 0; i < 16; i++) {
    ...
    const right = new THREE.Vector3(tan.z, 0, -tan.x).normalize();
    const lane = i % 4;
    const lat = lane === 0 ? 0 : lane === 1 ? 5.5 : lane === 2 ? -5.5 : (i % 2 ? 1 : -1) * (HALF_W + 3.4);
    const x = p.x + right.x * lat, z = p.z + right.z * lat;
    const kind = kinds[i % kinds.length];
    const mesh = new THREE.Mesh(
      new THREE.OctahedronGeometry(0.55),
      new THREE.MeshBasicMaterial({ color: cols[kind], toneMapped: false, fog: false })
    );
    mesh.position.set(x, terrainH(x, z) + 1.1, z);
    scene.add(mesh);
    pickups.push({ kind, x, z, mesh, taken: 0 });
  }
})();
function stepRivals(dt) {
  ...
  pickups.forEach((p) => {
    if (p.mesh) {
      ...
      p.mesh.position.y = terrainH(p.x, p.z) + 1.15 + Math.sin(timeMs * 0.005 + p.x) * 0.18;
    }
    if (p.taken > 0) { p.taken -= dt; if (p.taken <= 0 && p.mesh) p.mesh.visible = true; return; }
    if (Math.hypot(p.x - car.x, p.z - car.z) < 2.3) {
      grant(p.kind);
      p.taken = 9;
  ...
  for (let i = shots.length - 1; i >= 0; i--) {
    ...
    if (s.from === "player") {
      rivals.forEach((r) => {
        if (r.hp <= 0) return;
        if (Math.hypot(s.x - r.mesh.position.x, s.z - r.mesh.position.z) < 2.5) {
          r.hp -= s.dmg; r.spinT = 0.9; r.speed *= 0.4; hit = true;
          if (r.hp <= 0) { r.hp = 0; r.downT = 3.2; kos++; showToast("KO", 0.6); }
        }
      });
    } else if (car.spinT <= 0 && Math.hypot(s.x - car.x, s.z - car.z) < 2.3) {
      car.hp = Math.max(8, car.hp - s.dmg);
      spinOut(1.05);
      hit = true;
    }
  ...
  for (let i = mines.length - 1; i >= 0; i--) {
    ...
    if (m.arm > 0) continue;
    let boom = false;
    if (Math.hypot(m.x - car.x, m.z - car.z) < 2.6) { spinOut(1.15); car.hp = Math.max(8, car.hp - 22); boom = true; }
    rivals.forEach((r) => {
      if (r.hp <= 0) return;
      if (Math.hypot(m.x - r.mesh.position.x, m.z - r.mesh.position.z) < 2.6) {
        r.hp -= 28; r.spinT = 1; if (r.hp <= 0) { r.hp = 0; r.downT = 3.2; kos++; }
        boom = true;
      }
    });
    if (boom) { scene.remove(m.mesh); mines.splice(i, 1); }
  }
  rivals.forEach((r, i) => {
    if (r.hp <= 0) {
      r.mesh.rotation.z = 0.45;
      r.downT -= dt;
      if (r.downT <= 0) {
        r.hp = 80; r.spinT = 0; r.downT = 0;
        r.t = (r.t + 0.08) % 1;
        r.mesh.rotation.z = 0;
        r.speed = 78 + i * 6;
      }
    ...
    if (d < 2.7) {
      car.x += (car.x - x) / (d || 1) * 0.18;
      car.z += (car.z - z) / (d || 1) * 0.18;
    }
    r.fireT -= dt;
    if (r.fireT <= 0 && d < 38 && d > 6) {
      r.fireT = 2.2 + Math.random() * 1.6;
      const laser = i % 3 === 1;
      const sx = Math.sin(yaw), cz = Math.cos(yaw);
      const mesh = new THREE.Mesh(
        laser ? new THREE.CylinderGeometry(0.05, 0.05, 2.6, 6) : new THREE.SphereGeometry(0.13, 6, 6),
        new THREE.MeshBasicMaterial({ color: laser ? 0x66F0FF : 0xFFB020, toneMapped: false, fog: false })
      );
      if (laser) mesh.rotation.x = Math.PI / 2;
      const spd = laser ? 130 : 100;
      const s = {
        x: x + sx * 3, y: r.mesh.position.y + 1.1, z: z + cz * 3,
        vx: sx * spd, vz: cz * spd, life: laser ? 0.8 : 0.95,
        dmg: laser ? 22 : 14, mesh, from: "bot"
      };
      mesh.position.set(s.x, s.y, s.z);
      scene.add(mesh);
      shots.push(s);
...
function chaseCam(dt, boosting) {
  ...
  if (PROOF === "A") {
    ...
    const back = 14.5 + Math.min(10, spd * 0.055);
    const h = 5.4 + Math.min(4, spd * 0.02) + (car.air ? 2.2 : 0);
    const side = 2.2;
    _cam.set(car.x - sx * back + cz * side, car.y + h, car.z - cz * back - sx * side);
    const k = camReady ? 1 - Math.exp(-9 * dt) : 1;
    camera.position.lerp(_cam, k);
    _look.set(car.x + sx * 10, car.y + 1.5, car.z + cz * 10);
    camera.lookAt(_look);
    camera.fov += ((50 + Math.min(10, spd * 0.05) + (boosting ? 3 : 0) + (car.air ? 4 : 0)) - camera.fov) * Math.min(1, dt * 3);
  }
  const floor = terrainH(camera.position.x, camera.position.z) + 3.4;
...
function hudBind() {
  ...
  if (elHpFill) {
    elHpFill.style.width = car.hp + "%";
    elHpFill.style.background = car.hp < 30 ? "#e23b32" : "#3de8ff";
  }
  if (elItemVal) elItemVal.textContent = car.item === "NONE" ? "NONE" : (car.item + " x" + car.ammo);
  if (elKosVal) elKosVal.textContent = String(kos);
...
function loop() {
  ...
  if (!finished && Math.abs(car.speed) > 0.6) timeMs += dt * 1000;
