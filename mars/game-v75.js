...
// cache: game-v75.js  — 1v1 Cybertruck duel, flowing jumps, ram + turbo
const BUILD = "75";
const GATE_COUNT = 5;
...
const JUMPS = [
  { t: 0.12, kind: "whoops", h: 1.45, len: 62, n: 6 },
  { t: 0.26, kind: "table", h: 3.8, len: 68, n: 0 },
  { t: 0.42, kind: "kicker", h: 5.8, len: 58, n: 0 },
  { t: 0.58, kind: "whoops", h: 1.35, len: 56, n: 5 },
  { t: 0.72, kind: "table", h: 4.4, len: 72, n: 0 },
  { t: 0.88, kind: "kicker", h: 6.4, len: 62, n: 0 }
];
...
function jumpProfile(j, u) {
  ...
  if (j.kind === "kicker") {
    if (u < 0.38) return h * Math.pow(u / 0.38, 1.2);
    if (u < 0.46) return h;
    return h * Math.max(0, 1 - (u - 0.46) / 0.54);
  }
  if (j.kind === "table") {
  ...
  if (u < 0.72) return h;
  return h * Math.max(0, 1 - (u - 0.72) / 0.28);
}
function jumpYAtT(t) {
  const s = t * trackLen;
  let y = 0;
...
const gateTs = [0.08, 0.34, 0.50, 0.80, 0.96];
...
function drive(dt) {
  ...
  if (!car.air) {
    ...
    const crest = jNow > 2.6 && jFwd < jNow - 0.85;
    if (car.spinT <= 0 && spdAbs > 14 && crest) {
      car.air = true;
      car.vy = Math.min(18, 3.2 + jNow * 0.85 + spdAbs * 0.05);
      if (boosting || car.turboT > 0) car.vy *= 1.18;
    } else if (car.spinT <= 0 && spdAbs > 18 && jNow > 0.9 && jNow < 2.2 && jFwd < jNow - 0.4) {
      car.air = true;
      car.vy = Math.min(6, 2 + jNow * 0.6);
    }
  } else {
    car.vy -= GRAVITY * dt;
    car.y += car.vy * dt;
...
(function seedPickups() {
  const spots = [0.18, 0.33, 0.48, 0.63, 0.78];
  spots.forEach((t, i) => {
    const p = curve.getPointAt(t);
    const tan = curve.getTangentAt(t);
    const right = new THREE.Vector3(tan.z, 0, -tan.x).normalize();
