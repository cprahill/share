...
// cache: game-v76.js  — Excitebike gaps: auto-level air, ramp / hole / landing
const BUILD = "76";
const GATE_COUNT = 5;
...
const PIT = -2.35;
const JUMPS = [
  { t: 0.10, kind: "whoops", h: 1.25, len: 52, n: 5 },
  { t: 0.20, kind: "gap", h: 4.8, take: 20, gap: 18, land: 36, landH: 3.4 },
  { t: 0.36, kind: "table", h: 3.2, len: 58 },
  { t: 0.50, kind: "gap", h: 6.0, take: 24, gap: 22, land: 26, landH: 3.8, gap2: 16, land2: 34, landH2: 4.6 },
  { t: 0.72, kind: "whoops", h: 1.2, len: 48, n: 5 },
  { t: 0.84, kind: "gap", h: 5.4, take: 22, gap: 20, land: 38, landH: 3.6 }
];

function jumpLen(j) {
  if (j.kind === "gap") return j.take + j.gap + j.land + (j.gap2 || 0) + (j.land2 || 0);
  return j.len;
}
function jumpProfile(j, u) {
  ...
  if (j.kind === "whoops") {
    ...
    return h * win * (0.4 + 0.6 * Math.sin(u * j.n * Math.PI));
  }
  if (j.kind === "table") {
    if (u < 0.22) return h * (u / 0.22);
    if (u < 0.64) return h;
    return h * Math.max(0, 1 - (u - 0.64) / 0.36);
  }
  if (j.kind === "gap") {
    const tot = jumpLen(j);
    const x = u * tot;
    if (x <= j.take) return h * Math.pow(Math.max(0, x / j.take), 1.12);
    if (x <= j.take + j.gap) return PIT;
    if (x <= j.take + j.gap + j.land) {
      const k = (x - j.take - j.gap) / j.land;
      return j.landH * (1 - k);
    }
    if (j.gap2 && x <= j.take + j.gap + j.land + j.gap2) return PIT;
    if (j.land2) {
      const k = (x - j.take - j.gap - j.land - (j.gap2 || 0)) / j.land2;
      return j.landH2 * Math.max(0, 1 - k);
    }
    return 0;
  }
  return 0;
...
function inPitT(t) { return jumpYAtT(t) < -1.1; }
...
for (let i = 0; i <= ribbonN; i++) {
  ...
  const pit = inPitT(t);
  const h = pit ? terrainH(p.x, p.z) + RIBBON_LIFT : terrainH(p.x, p.z) + RIBBON_LIFT;
  ribbonPos.push(p.x - r.x * (HALF_W - 0.3), h, p.z - r.z * (HALF_W - 0.3));
  ribbonPos.push(p.x + r.x * (HALF_W - 0.3), h, p.z + r.z * (HALF_W - 0.3));
  ribbonUv.push(t * 40, 0, t * 40, 1);
}
for (let i = 0; i < ribbonN; i++) {
  const t0 = i / ribbonN, t1 = (i + 1) / ribbonN;
  if (inPitT(t0) || inPitT(t1)) continue;
  const i0 = i * 2;
...
function chevronTex(a, b) {
  const c = document.createElement("canvas");
  c.width = 256; c.height = 64;
  const g = c.getContext("2d");
  g.fillStyle = a; g.fillRect(0, 0, 256, 64);
  g.fillStyle = b;
  for (let i = 0; i < 8; i++) {
    g.beginPath();
    const x = i * 32;
    g.moveTo(x, 0); g.lineTo(x + 22, 32); g.lineTo(x, 64); g.closePath(); g.fill();
  }
  const t = new THREE.CanvasTexture(c);
  t.wrapS = t.wrapT = THREE.RepeatWrapping;
  t.repeat.set(4, 1);
...
const TEX_TAKE = chevronTex("#E8B923", "#3a2218");
const TEX_NEAR = chevronTex("#f4f4f2", "#8A4A32");
const TEX_FAR = chevronTex("#66F0FF", "#1a3038");

function paintRamp(j, fromM, toM, map) {
  const n = Math.max(8, ((toM - fromM) / 2) | 0);
  const pos = [], idx = [], uv = [];
  const start = j.t * trackLen;
  for (let i = 0; i <= n; i++) {
    const x = fromM + (toM - fromM) * (i / n);
    const t = ((start + x) / trackLen % 1 + 1) % 1;
    const p = curve.getPointAt(t);
    const tan = curve.getTangentAt(t);
    const r = new THREE.Vector3(tan.z, 0, -tan.x).normalize();
    const yTop = terrainH(p.x, p.z);
    const rise = Math.max(0, jumpProfile(j, x / jumpLen(j)));
    const yBot = yTop - rise - 0.35;
    const hw = HALF_W + 0.35;
    pos.push(
  ...
  const mesh = new THREE.Mesh(geo, new THREE.MeshStandardMaterial({
    map, color: 0xffffff, roughness: 0.85, metalness: 0.04, flatShading: true
  }));
...
(function markJumps() {
  function stripeAt(s, hex, w) {
    const t = ((s / trackLen) % 1 + 1) % 1;
    const p = curve.getPointAt(t);
    const tan = curve.getTangentAt(t);
    const y = terrainH(p.x, p.z) + 0.2;
    const stripe = new THREE.Mesh(
      new THREE.BoxGeometry(HALF_W * 2 - 0.5, 0.16, w || 1.2),
  ...
  JUMPS.forEach((j) => {
    const start = j.t * trackLen;
    if (j.kind === "gap") {
      stripeAt(start + j.take - 1, 0xFFEE55, 1.4);
      stripeAt(start + j.take + j.gap + 1, 0xf4f4f2, 1.3);
      if (j.gap2) stripeAt(start + j.take + j.gap + j.land + j.gap2 + 1, 0x66F0FF, 1.4);
      paintRamp(j, 0, j.take, TEX_TAKE);
      paintRamp(j, j.take + j.gap, j.take + j.gap + j.land, TEX_NEAR);
      if (j.land2) paintRamp(j, j.take + j.gap + j.land + j.gap2, jumpLen(j), TEX_FAR);
    } else if (j.kind === "table") {
      stripeAt(start + j.len * 0.22, 0xE8B923, 1.0);
      paintRamp(j, 0, j.len, TEX_TAKE);
    }
  });
})();
...
const gateTs = [0.07, 0.32, 0.46, 0.68, 0.98];
...
function drive(dt) {
  ...
  const onTake = hit && hit.j.kind === "gap" && hit.x >= hit.j.take * 0.78 && hit.x <= hit.j.take + 0.8;
  if (!car.air) {
    car.y = groundY;
    car.vy = 0;
    const wantPitch = THREE.MathUtils.clamp(Math.atan2(yA - yB, 6.4), -0.55, 0.55);
    car.pitch += (wantPitch - car.pitch) * Math.min(1, dt * 9);
    if (car.spinT <= 0 && spdAbs > 16 && onTake) {
      const ang = Math.atan2(hit.j.h, Math.max(8, hit.j.take));
      car.air = true;
      car.vy = Math.sin(ang) * spdAbs * 1.12 + (boosting || car.turboT > 0 ? 5.5 : 1.2);
      car.speed += boosting || car.turboT > 0 ? 7 : 3;
      showToast(boosting || car.turboT > 0 ? "LONG" : "JUMP", 0.35);
    }
    if (!car.air && jumpYAtT(proj.t) < -1.15) {
    ...
    if (car.y <= groundY) {
      const pitLand = jumpYAtT(proj.t) < -1.15;
      car.y = groundY;
      car.vy = 0;
      car.air = false;
      if (pitLand) {
        ...
        if (mismatch > 0.55 && spdAbs > 10) spinOut(0.85);
        else if (mismatch < 0.22) {
          car.speed = Math.min(vmax, car.speed + 10);
          car.boost = Math.min(1, car.boost + 0.16);
        } else car.speed *= 0.8;
        emitDust(car.x, car.y + 0.2, car.z, fx, fz, 12);
      }
    }
  }
...
function stepRivals(dt) {
  ...
  if (r.spinT > 0) {
    ...
    if (inPitT(r.t)) r.t = (r.t + 10 / trackLen) % 1;
