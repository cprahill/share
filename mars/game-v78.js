...
// cache: game-v78.js  — 3-lap Mars Baja look from concept: pale road, rust dunes, volcano
const BUILD = "78";
const GATE_COUNT = 5;
const LAP_COUNT = 3;
...
scene.fog = new THREE.FogExp2(FOG_PEACH, 0.00105);
...
(function marsLand() {
  ...
  function plantDune(x, z) {
    ...
    dummy.position.set(x, groundH(x, z) - s * 0.38, z);
    dummy.scale.set(s * (1.6 + Math.random()), s * 0.42, s * (0.9 + Math.random() * 0.5));
  ...
  [[-380, 500, 90], [-420, 900, 110], [-360, 1300, 80], [980, 700, 95], [1020, 1100, 120]].forEach(([x, z, h]) => {
    ...
    ridge.position.set(x, groundH(x, z) + h * 0.35, z);
...
{
  const mat = new THREE.MeshStandardMaterial({ color: 0xA05030, roughness: 0.97, metalness: 0.02, fog: false });
  const shield = new THREE.Mesh(new THREE.CylinderGeometry(48, 160, 210, 32, 5, true), mat);
  shield.position.y = 80;
  olympus.add(shield);
  const cap = new THREE.Mesh(new THREE.CylinderGeometry(48, 58, 18, 24), new THREE.MeshStandardMaterial({
    color: 0xB86840, roughness: 0.9, fog: false
  }));
  cap.position.y = 178;
  olympus.add(cap);
}
olympus.position.set(780, 0, 1180);
...
(function scatterRocks() {
  ...
  const mat = new THREE.MeshStandardMaterial({ color: 0x6A3224, roughness: 0.95, metalness: 0.02, flatShading: true });
  clusterAlong(0.14, 0.38, 1, 18, 34, 16, (x, z, k) => {
    const s = 0.65 + (k % 4) * 0.4;
    if (minDistToPath(x, z) < HALF_W + 6) return;
    const m = new THREE.Mesh(geo, mat);
    m.scale.set(s, s * 0.65, s);
    m.position.set(x, groundH(x, z) + s * 0.28, z);
    m.rotation.set(k * 0.4, k * 1.1, 0.2);
    m.castShadow = true;
    scene.add(m);
  });
})();
