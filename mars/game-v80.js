...
// cache: game-v80.js  — sky no longer overdraws the road; trench + lift so ribbon never z-fights
const BUILD = "80";
const GATE_COUNT = 5;
const LAP_COUNT = 3;
...
const camera = new THREE.PerspectiveCamera(52, innerWidth / innerHeight, 2, 2800);
scene.add(camera);
...
const DIRT_TRACK = dirtTexture(6, "#C8B090");
...
const ribbon = new THREE.Mesh(ribbonGeo, new THREE.MeshStandardMaterial({
  map: DIRT_TRACK, color: 0xB89870, roughness: 0.92, metalness: 0.04,
...
(function marsLand() {
  ...
  const duneMat = new THREE.MeshStandardMaterial({ color: 0xC05632, roughness: 0.98, metalness: 0.0 });
  const dunes = new THREE.InstancedMesh(duneGeo, duneMat, 40);
  ...
  function plantDune(x, z) {
    ...
    const sy = s * 0.42;
    dummy.scale.set(s * (1.6 + Math.random()), sy, s * (0.9 + Math.random() * 0.5));
    dummy.position.set(x, groundH(x, z) + sy * 0.55, z);
    dummy.rotation.y = Math.random() * 0.8;
    dummy.updateMatrix();
    dunes.setMatrixAt(n++, dummy.matrix);
  }
  clusterAlong(0.12, 0.42, -1, 70, 160, 22, plantDune);
  clusterAlong(0.58, 0.86, 1, 80, 170, 18, plantDune);
  dunes.count = n;
  dunes.instanceMatrix.needsUpdate = true;
  scene.add(dunes);

  const ridgeMat = new THREE.MeshStandardMaterial({ color: 0x8A4A32, roughness: 0.96, metalness: 0.02, flatShading: true, fog: true });
  [[-380, 500, 90], [-420, 900, 110], [-360, 1300, 80], [980, 700, 95], [1020, 1100, 120]].forEach(([x, z, h]) => {
    if (minDistToPath(x, z) < 120) return;
    const ridge = new THREE.Mesh(new THREE.ConeGeometry(h * 0.9, h, 6), ridgeMat);
    ridge.position.set(x, groundH(x, z) + h * 0.48, z);
...
{
  const mat = new THREE.MeshStandardMaterial({ color: 0xA05030, roughness: 0.97, metalness: 0.02, fog: true });
  const shield = new THREE.Mesh(new THREE.CylinderGeometry(48, 160, 210, 32, 5, false), mat);
  shield.position.y = 80;
  shield.castShadow = true;
  shield.receiveShadow = true;
  olympus.add(shield);
  const cap = new THREE.Mesh(new THREE.CylinderGeometry(48, 58, 18, 24), new THREE.MeshStandardMaterial({
    color: 0xB86840, roughness: 0.9, fog: true
  }));
  cap.position.y = 178;
  olympus.add(cap);
}
olympus.position.set(780, groundH(780, 1180) - 8, 1180);
olympus.scale.setScalar(1.35);
...
function plantStl(url, x, z, worldH, hex, rotY) {
  const loader = new STLLoader();
  loader.load(url, (geo) => {
    geo.computeVertexNormals();
    geo.computeBoundingBox();
    ...
    const mesh = new THREE.Mesh(geo, new THREE.MeshStandardMaterial({
      color: hex, roughness: 0.94, metalness: 0.04, side: THREE.DoubleSide
    }));
    ...
    if (minDistToPath(x, z) - foot * 0.35 < HALF_W + 10) return;
    mesh.position.y = groundH(x, z) - box.min.y - 0.15;
    scene.add(mesh);
    const blob = new THREE.Mesh(
      new THREE.CircleGeometry(foot * 0.28, 10),
      new THREE.MeshBasicMaterial({ color: 0x000000, transparent: true, opacity: 0.28, depthWrite: false })
    );
    blob.rotation.x = -Math.PI / 2;
    blob.position.set(x, groundH(x, z) + 0.04, z);
    scene.add(blob);
    addCollider(x, z, Math.max(1.4, foot * 0.22));
  });
}
(function scatterRocks() {
  const urls = ["./mesh/rock-a.stl", "./mesh/rock-b.stl", "./mesh/rock-c.stl", "./mesh/rock-d.stl"];
  const cols = [0xA6583D, 0x8B3A2B, 0x733021, 0x5A2818];
  urls.forEach((url, vi) => {
    clusterAlong(0.12 + vi * 0.08, 0.34 + vi * 0.12, vi % 2 ? 1 : -1, 20, 40, 3, (x, z, k) => {
      plantStl(url, x, z, 1.6 + (k % 3) * 0.7, cols[vi], k * 1.3 + vi);
    });
  });
  plantStl("./mesh/mountain-a.stl", -320, 640, 55, 0x8B3A2B, 0.4);
  plantStl("./mesh/mountain-b.stl", -300, 1240, 62, 0x7A3224, 1.1);
  plantStl("./mesh/mountain-c.stl", 920, 980, 58, 0x8A4A32, 0.2);
  plantStl("./mesh/hill-a.stl", -90, 380, 12, 0xC05632, 0.2);
  plantStl("./mesh/hill-b.stl", 540, 620, 14, 0xA6583D, 1.4);
  plantStl("./mesh/hill-c.stl", 80, 980, 11, 0x8B3A2B, 0.8);
})();
...
(function seedPickups() {
  ...
  spots.forEach((t, i) => {
    ...
    mesh.add(core);
    mesh.add(halo);
    mesh.position.set(x, terrainH(x, z) + 2.1, z);
    scene.add(mesh);
...
function stepRivals(dt) {
  ...
  pickups.forEach((p) => {
    if (p.mesh) {
      ...
      p.mesh.position.y = terrainH(p.x, p.z) + 2.1 + Math.sin(timeMs * 0.006 + p.x) * 0.35;
    ...
    if (!startLock && Math.hypot(p.x - car.x, p.z - car.z) < 3.4) {
      grant(p.kind);
      p.taken = 11;
