...
// cache: game-v84.js  — colony spread, ship gauntlet, solar farm, crowds, flyers
const BUILD = "84";
const GATE_COUNT = 5;
const LAP_COUNT = 3;
...
{
  let coarse = false;
  try { coarse = matchMedia("(pointer: coarse)").matches; } catch (err) {}
...
scene.fog = new THREE.Fog(FOG_PEACH, 650, 3200);
...
const camera = new THREE.PerspectiveCamera(52, innerWidth / innerHeight, 2, 4200);
scene.add(camera);
...
function packedTrackTexture() {
  ...
  rut(0.30, 0.11, 0.38);
  rut(0.70, 0.11, 0.38);
  g.fillStyle = "rgba(110, 88, 64, 0.16)";
  g.fillRect(0, 0.46 * 512, 512, 0.08 * 512);
  for (let i = 0; i < 700; i++) {
...
function trackBumpTexture() {
  ...
  g.fillStyle = "#5a5a5a";
  g.fillRect(0, 0.26 * 256, 256, 0.12 * 256);
  g.fillRect(0, 0.64 * 256, 256, 0.12 * 256);
...
const DIRT_GROUND = dirtTexture(48, "#C05632");
const DIRT_TRACK = packedTrackTexture();
const DIRT_TRACK_BUMP = trackBumpTexture();
const DIRT_SHOULDER = dirtTexture(10, "#A05632");
...
for (let i = 0; i <= ribbonN; i++) {
  ...
  const hw = HALF_W - 0.2;
  ribbonPos.push(p.x - r.x * hw, h, p.z - r.z * hw);
  ribbonPos.push(p.x, h + 0.14, p.z);
  ribbonPos.push(p.x + r.x * hw, h, p.z + r.z * hw);
  const u = t * 28;
  ribbonUv.push(u, 0, u, 0.5, u, 1);
}
for (let i = 0; i < ribbonN; i++) {
  const a = i * 3, b = (i + 1) * 3;
  ribbonIdx.push(a, b, a + 1, a + 1, b, b + 1, a + 1, b + 1, a + 2, a + 2, b + 1, b + 2);
}
...
const ribbon = new THREE.Mesh(ribbonGeo, new THREE.MeshStandardMaterial({
  map: DIRT_TRACK, bumpMap: DIRT_TRACK_BUMP, bumpScale: 0.55,
  color: 0xC4A882, roughness: 0.88, metalness: 0.03,
  polygonOffset: true, polygonOffsetFactor: -4, polygonOffsetUnits: -8, fog: false
}));
...
function addDirtStrip(offA, offB, lift, mat) {
  const pos = [], uv = [], idx = [];
  const n = 800;
  const span = Math.abs(offB - offA) || 1;
  for (let i = 0; i <= n; i++) {
    const t = i / n;
    const p = curve.getPointAt(t);
    const tan = curve.getTangentAt(t);
    const r = new THREE.Vector3(tan.z, 0, -tan.x).normalize();
    const h = trackH(p.x, p.z) + lift;
    pos.push(p.x + r.x * offA, h, p.z + r.z * offA, p.x + r.x * offB, h, p.z + r.z * offB);
    uv.push(t * 22, 0, t * 22, span * 0.25);
  }
  for (let i = 0; i < n; i++) {
    const i0 = i * 2;
...
const bermMat = new THREE.MeshStandardMaterial({
  map: DIRT_SHOULDER, color: BERM_RUST, roughness: 0.96, metalness: 0.02, fog: false
});
const shoulderMat = new THREE.MeshStandardMaterial({
  map: DIRT_SHOULDER, color: 0xB06038, roughness: 0.97, metalness: 0.02, fog: false
});
addDirtStrip(-(HALF_W + 0.05), -(HALF_W - 0.85), RIBBON_LIFT + 0.42, bermMat);
addDirtStrip((HALF_W - 0.85), (HALF_W + 0.05), RIBBON_LIFT + 0.42, bermMat);
addDirtStrip(-(HALF_W + 2.8), -(HALF_W + 0.05), RIBBON_LIFT - 0.06, shoulderMat);
addDirtStrip((HALF_W + 0.05), (HALF_W + 2.8), RIBBON_LIFT - 0.06, shoulderMat);
...
(function marsLand() {
  ...
  const duneGeo = new THREE.SphereGeometry(1, 28, 18);
  {
  ...
  clusterAlong(0.08, 0.22, -1, 110, 200, 10, plantDune);
  clusterAlong(0.30, 0.46, -1, 120, 210, 10, plantDune);
  clusterAlong(0.58, 0.70, 1, 110, 200, 9, plantDune);
  clusterAlong(0.72, 0.86, 1, 115, 205, 9, plantDune);
  clusterAlong(0.90, 0.99, -1, 100, 170, 6, plantDune);
  dunes.count = n;
...
(function starshipGarden() {
  ...
  const vaultGlass = new THREE.Mesh(
    new THREE.CylinderGeometry(vaultR, vaultR, vaultLen, 32, 1, true, Math.PI / 2, Math.PI),
    glassMatDome
  );
  ...
  loadStl("./mesh/worker.stl", (geo) => {
    const spots = [
      [vaultR + 5.2, -12, 1.72, 0.4, workerMat],
      [vaultR + 8.5, 4, 1.70, 2.6, workerMatB],
      [vaultR + 6.8, 17, 1.74, -0.8, workerMat],
      [vaultR + 11.2, -6, 1.68, 1.2, workerMatB],
      [-(vaultR + 6.4), 8, 1.70, 2.2, workerMat],
      [-(vaultR + 9.0), -14, 1.72, -0.5, workerMatB]
    ];
    spots.forEach((s) => {
      sitMesh(geo, wx(s[0], s[1]), wz(s[0], s[1]), s[2], s[4], s[3], { collider: 0.55, bury: 0.02, clear: 5 });
    });
  });
  loadStl("./mesh/woman.stl", (geo) => {
    const spots = [
      [vaultR + 7.4, 10, 1.68, 3.4],
      [vaultR + 10.0, -18, 1.66, 0.6],
      [-(vaultR + 7.8), 16, 1.66, 1.1]
    ];
    spots.forEach((s) => {
      sitMesh(geo, wx(s[0], s[1]), wz(s[0], s[1]), s[2], womanMat, s[3], { collider: 0.5, bury: 0.02, clear: 5 });
    });
  });
})();
...
function makeAstronaut(hex) {
  ...
  const torso = new THREE.Mesh(new THREE.CapsuleGeometry(0.22, 0.52, 4, 8), suit);
  torso.position.y = 0.82;
  const helm = new THREE.Mesh(new THREE.SphereGeometry(0.2, 10, 8), visor);
...
function plantCrowd(t, side, dist, n) {
  ...
  const pole = new THREE.Mesh(new THREE.CylinderGeometry(0.05, 0.05, 2.4, 6), trussMat);
  pole.position.set(0, groundH(at.x, at.z) + 1.2, -0.7);
  const flag = new THREE.Mesh(new THREE.BoxGeometry(0.04, 0.7, 1.1), bannerMat);
  flag.position.set(0, groundH(at.x, at.z) + 2.05, -0.15);
  g.add(pole, flag);
  scene.add(g);
...
(function worldDress() {
  ...
  const farm = new THREE.InstancedMesh(panelGeo, solarMat, 72);
  ...
  plantCrowd(0.12, -1, 22, 4);
  plantCrowd(0.21, 1, 24, 3);
  plantCrowd(0.29, -1, 23, 4);
  plantCrowd(0.41, 1, 26, 5);
  plantCrowd(0.52, -1, 30, 4);
  plantCrowd(0.63, 1, 24, 3);
  plantCrowd(0.76, -1, 25, 4);
  plantCrowd(0.88, 1, 22, 3);
...
(function seedPickups() {
  ...
  addPickup("TURBO", 0.15, 5.2, 2.1, 11);
  addPickup("TURBO", 0.33, -5.0, 2.1, 11);
  addPickup("TURBO", 0.40, 4.6, 2.1, 11);
  addPickup("TURBO", 0.53, 0, 2.4, 11);
  addPickup("TURBO", 0.66, 5.0, 2.1, 11);
  addPickup("TURBO", 0.80, -4.8, 2.1, 11);
  JUMPS.forEach((j) => {
...
function stepRivals(dt) {
  ...
  pickups.forEach((p) => {
    ...
    if (p.kind === "RING" && Math.abs(car.y - (p.y || 0)) > 5.5) return;
    grant(p.kind);
    p.taken = p.cool || 8;
