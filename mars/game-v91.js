...
// cache: game-v91.js  — real GLB dress, people off the pad
const BUILD = "91";
...
function nearRocket(x, z) {
  const dx = x - PAD_ROCKET_X, dz = z - PAD_ROCKET_Z;
  return dx * dx + dz * dz < ROCKET_KEEP_R * ROCKET_KEEP_R;
}

const DIFFS = {
  easy: { key: "easy", tag: "EASY", nGround: 0, nAir: 0, airHz: 0, lat0: 7.2, lat1: 9.5, slow: 0.64, spin: false, rival: [54, 58, 46] },
  medium: { key: "medium", tag: "MED", nGround: 8, nAir: 0, airHz: 0, lat0: 5.4, lat1: 8.4, slow: 0.52, spin: false, rival: [60, 64, 50] },
  hard: { key: "hard", tag: "HARD", nGround: 16, nAir: 6, airHz: 1.45, lat0: 3.8, lat1: 7.2, slow: 0.40, spin: true, rival: [66, 72, 52] },
  extra: { key: "extra", tag: "XHARD", nGround: 16, nAir: 6, airHz: 2.7, lat0: 3.8, lat1: 7.2, slow: 0.40, spin: true, rival: [66, 72, 52] }
};
...
function onPtrDown(e) {
  ...
  if (e.target && e.target.closest && e.target.closest("#boot, #share-chal, .boot-btn, #hs-name, #hs-save, #hs-board, #finish, #boot-name, #paint-row")) return;
...
scene.add(new THREE.HemisphereLight(0xF8C8A0, 0xA04828, 1.55));
scene.add(new THREE.AmbientLight(0xE09060, 0.7));
const sun = new THREE.DirectionalLight(0xFFD0A0, 4.6);
...
function dirtTexture(rep, hex) {
  ...
  for (let i = 0; i < 900; i++) {
    ...
    g.fillStyle = "rgba(" + (90 + v) + "," + (48 + v) + "," + (36 + (v * 0.4) | 0) + "," + (0.22 + Math.random() * 0.25) + ")";
    const s = 2 + Math.random() * 5;
    g.fillRect(Math.random() * 512, Math.random() * 512, s, s * (0.4 + Math.random() * 0.6));
  }
...
function packedTrackTexture() {
  ...
  for (let i = 0; i < 16000; i++) {
    ...
    g.fillStyle = "rgba(" + (150 + v) + "," + (120 + (v * 0.7) | 0) + "," + (80 + (v * 0.4) | 0) + "," + (0.12 + Math.random() * 0.28) + ")";
    g.fillRect(Math.random() * 512, Math.random() * 512, 1 + Math.random() * 3, 1 + Math.random() * 3);
  }
  // Tire ruts run along U, sit across V (ribbon v = 0..1 is left..right).
  function rut(v0, w, alpha) {
    g.fillStyle = "rgba(92, 68, 48," + alpha + ")";
    g.fillRect(0, (v0 - w * 0.5) * 512, 512, w * 512);
    g.fillStyle = "rgba(168, 140, 108, 0.18)";
    g.fillRect(0, (v0 - w * 0.18) * 512, 512, w * 0.22 * 512);
  }
  rut(0.38, 0.07, 0.16);
  rut(0.62, 0.07, 0.16);
  for (let i = 0; i < 700; i++) {
    const v = (Math.random() * 30) | 0;
    g.fillStyle = "rgba(" + (80 + v) + "," + (62 + v) + "," + (48 + (v * 0.5) | 0) + ",0.35)";
    g.fillRect(Math.random() * 512, Math.random() * 512, 2 + Math.random() * 4, 1 + Math.random() * 2);
  }
  const t = new THREE.CanvasTexture(c);
  t.wrapS = t.wrapT = THREE.RepeatWrapping;
  t.repeat.set(1, 1);
...
const DIRT_GROUND = dirtTexture(IS_MOBILE ? 24 : 48, "#C05632");
const DIRT_TRACK = packedTrackTexture();
...
const LIVERIES = [
  { name: "STEEL", color: 0x6E7378, metalness: 0.18, roughness: 0.46, env: 0.35 },
  { name: "BLACK", color: 0x161618, metalness: 0.58, roughness: 0.48, env: 0.70 },
  { name: "WHITE", color: 0xD8D4CC, metalness: 0.82, roughness: 0.22, env: 1.05 },
  { name: "RED", color: 0xB42018, metalness: 0.52, roughness: 0.42, env: 0.80 },
  { name: "GOLD", color: 0xC9A227, metalness: 0.85, roughness: 0.28, env: 1.10 },
  { name: "TEAL", color: 0x1A7A7A, metalness: 0.55, roughness: 0.40, env: 0.85 },
  { name: "BLUE", color: 0x2A4A8C, metalness: 0.60, roughness: 0.38, env: 0.90 },
  { name: "TAN", color: 0x8C7360, metalness: 0.68, roughness: 0.54, env: 0.82 }
];
let liveryIdx = 0;
try {
...
const ground = new THREE.Mesh(
  ...
  new THREE.MeshStandardMaterial({ map: DIRT_GROUND, color: 0xC05632, roughness: 0.97, metalness: 0.02 })
);
...
const XSEC = [
  ...
  { o: -(HALF_W - 0.45), dh: 0.04, v: 0.30, c: [0.80, 0.67, 0.51] },
  { o: 0, dh: 0.06, v: 0.50, c: [0.82, 0.70, 0.54] },
  { o: (HALF_W - 0.45), dh: 0.04, v: 0.70, c: [0.80, 0.67, 0.51] },
...
const sky = new THREE.Mesh(
  ...
  new THREE.ShaderMaterial({
    ...
    fragmentShader: "varying vec3 w; void main(){ float h = normalize(w).y; vec3 hor = vec3(0.96,0.68,0.48); vec3 zen = vec3(0.78,0.50,0.40); gl_FragColor = vec4(mix(hor, zen, max(h,0.0)*0.85),1.0); }"
  })
);
...
scene.add(sky);
{
  ...
  sunMesh.position.set(-520, 180, -280);
...
(function marsLand() {
  ...
  function plantDune(x, z) {
    if (n >= 56) return;
    if (minDistToPath(x, z) < HALF_W + 36) return;
  ...
  clusterAlong(0.08, 0.22, -1, 110, 200, IS_MOBILE ? 5 : 10, plantDune);
  clusterAlong(0.30, 0.46, -1, 120, 210, IS_MOBILE ? 5 : 10, plantDune);
  clusterAlong(0.58, 0.70, 1, 110, 200, IS_MOBILE ? 5 : 9, plantDune);
  clusterAlong(0.72, 0.86, 1, 115, 205, IS_MOBILE ? 4 : 9, plantDune);
  clusterAlong(0.90, 0.99, -1, 100, 170, IS_MOBILE ? 3 : 6, plantDune);
  dunes.count = n;
...
function sitMesh(geo, x, z, worldH, mat, rotY, opts) {
  ...
  const clear = opts.clear != null ? opts.clear : 8;
  if (!opts.allowPath && minDistToPath(x, z) - foot * 0.28 < HALF_W + clear) return null;
...
(function sceneSections() {
  ...
  loadStl("./mesh/rock-a.stl", (geo) => {
    const spots = [
      [0.09, -1, 26, 2.6, 0.4], [0.13, -1, 32, 3.2, 1.2], [0.16, 1, 29, 2.4, 2.0],
      [0.19, -1, 28, 2.8, 0.7], [0.24, 1, 34, 3.0, 1.5],
  ...
  loadStl("./mesh/rock-c.stl", (geo) => {
    const spots = [
      [0.11, 1, 40, 2.2, 0.2], [0.15, -1, 36, 2.8, 1.1], [0.21, 1, 30, 2.4, 0.7],
...
(function starshipGarden() {
  ...
  const leanW = 14, leanLen = 70, leanH = 8.4;
  [-1, 1].forEach((side) => {
...
function sitGlb(root, x, z, worldH, rotY, opts) {
  opts = opts || {};
  if (opts.keepOffRocket && nearRocket(x, z)) return null;
  ...
  const foot = Math.max(box.max.x - box.min.x, box.max.z - box.min.z);
  const clear = opts.clear != null ? opts.clear : 7;
  if (!opts.allowPath && minDistToPath(x, z) - foot * 0.28 < HALF_W + clear) return null;
  ...
  if (opts.collider) addCollider(x, z, opts.collider === true ? Math.max(1.1, foot * 0.38) : opts.collider);
  return root;
}
function plantCrowd(t, side, dist, n) {
  const at = alongTrack(t, side, dist);
  if (nearRocket(at.x, at.z)) return;
  const g = new THREE.Group();
  g.position.set(at.x, 0, at.z);
  g.rotation.y = at.yaw + (side > 0 ? Math.PI : 0);
  const colors = [0xf2f0ea, 0xe8dcc8, 0xd0d4da, 0xc45c2a];
  for (let i = 0; i < n; i++) {
...
function plantHabitat(t, side, dist) {
  ...
  g.add(tank);
  const ast = makeAstronaut(0xf2f0ea);
...
(function worldDress() {
  ...
  const hut = new THREE.Mesh(new THREE.BoxGeometry(4.2, 2.2, 3.2), new THREE.MeshStandardMaterial({ color: 0x8a8e94, metalness: 0.35, roughness: 0.5 }));
  hut.position.set(solarAt.x, groundH(solarAt.x, solarAt.z) + 1.1, solarAt.z);
  hut.rotation.y = solarAt.yaw;
  hut.frustumCulled = false;
  scene.add(hut);

  plantCrowd(0.12, -1, 22, IS_MOBILE ? 2 : 4);
  plantCrowd(0.28, -1, 23, IS_MOBILE ? 2 : 4);
  plantCrowd(0.41, 1, 26, IS_MOBILE ? 3 : 5);
  plantCrowd(0.63, 1, 24, IS_MOBILE ? 2 : 3);
  plantCrowd(0.88, 1, 22, IS_MOBILE ? 2 : 3);
  if (!IS_MOBILE) {
    plantCrowd(0.21, 1, 24, 3);
    plantCrowd(0.52, -1, 30, 4);
    plantCrowd(0.76, -1, 25, 4);
    plantCrowd(0.96, -1, 23, 4);
  }

  plantHabitat(0.19, 1, 38);
  plantHabitat(0.31, -1, 40);
  plantHabitat(0.45, 1, 42);
  plantHabitat(0.61, -1, 38);
  plantHabitat(0.80, 1, 40);
  if (!IS_MOBILE) plantHabitat(0.93, -1, 36);

  plantRover(0.18, 1, 34);
  plantRover(0.68, -1, 32);

  const flameO = new THREE.MeshBasicMaterial({
    color: 0xff7a28, transparent: true, opacity: 0.7, fog: false, toneMapped: false, side: THREE.DoubleSide, depthWrite: false
  });
  const flameI = new THREE.MeshBasicMaterial({
    color: 0xffe9a8, transparent: true, opacity: 0.86, fog: false, toneMapped: false, side: THREE.DoubleSide, depthWrite: false
  });
  ...
  loadStl("./mesh/obj_3_Ship.stl", (geo) => {
    ...
    function placeShip(x, z, worldH, rotY, mode) {
      ...
      if (mode === "hover") {
        worldAnim.flyers.push({ g, exhaust: ex, phase: "hover", t: Math.random() * 4, baseY: g.position.y, worldH });
      } else if (mode === "fly") {
        worldAnim.flyers.push({ g, exhaust: ex, phase: Math.random() < 0.5 ? "pad" : "coast", t: Math.random() * 8, baseY: g.position.y, worldH });
      }
      return g;
    }
    const gant = IS_MOBILE
      ? [[0.50, -1, 27, 40, 0.1], [0.52, 1, 28, 44, 0.4], [0.55, -1, 26, 42, 1.1], [0.57, 1, 27, 41, 0.2]]
      : [
        [0.49, -1, 26, 42, 0.1], [0.505, 1, 27, 46, 0.4],
        [0.52, -1, 25, 40, 1.1], [0.535, 1, 28, 48, 0.2],
        [0.55, -1, 26, 44, 0.7], [0.565, 1, 25, 41, 1.4]
      ];
    gant.forEach((s, i) => {
      const at = alongTrack(s[0], s[1], s[2]);
      const mode = i === 2 ? "hover" : "land";
      placeShip(at.x, at.z, s[3], at.yaw + s[4], mode);
    });
    const mid = alongTrack(0.53, 0, 0);
    worldAnim.landmarks.push({ kind: "GAUNTLET", x: mid.x, z: mid.z, r: 42, cool: 0 });
...
function dressColony() {
  const nMobile = IS_MOBILE;
  function scatter(url, spots, worldH, paint, collider) {
    loadGlb(url, (tmpl) => {
      spots.forEach((s, i) => {
        if (nMobile && i % 2) return;
        const at = alongTrack(s[0], s[1], s[2]);
        const root = cloneGlb(tmpl);
        if (paint) marsPaint(root, paint);
        sitGlb(root, at.x, at.z, s[3] != null ? s[3] : worldH, at.yaw + (s[4] || 0) + i * 0.11, {
          collider: collider, bury: 0.12, clear: 6, keepOffRocket: true
        });
      });
    });
  }
  scatter("./lib/nature-rock-a.glb", [
    [0.09, -1, 32, 3.4, 0.2], [0.16, 1, 36, 4.1, 1.1], [0.24, -1, 40, 3.6, 0.4],
    [0.48, 1, 34, 4.4, 0.8], [0.70, -1, 38, 3.8, 1.4], [0.91, 1, 33, 4.0, 0.3]
  ], 3.8, 0.5, true);
  scatter("./lib/nature-rock-b.glb", [
    [0.13, 1, 38, 3.2, 0.6], [0.36, -1, 42, 4.6, 1.7], [0.57, 1, 36, 3.5, 0.2],
    [0.79, -1, 40, 4.2, 0.9], [0.95, -1, 34, 3.3, 1.2]
  ], 3.6, 0.48, true);
  scatter("./lib/nature-rock-c.glb", [
    [0.20, -1, 30, 2.8, 0.4], [0.44, 1, 31, 3.1, 1.0], [0.66, 1, 29, 2.6, 0.7], [0.84, -1, 32, 3.0, 0.2]
  ], 2.9, 0.45, true);
  scatter("./lib/sandstone-boulder.glb", [
    [0.11, 1, 48, 2.4, 0.3], [0.27, -1, 52, 3.0, 1.4], [0.54, 1, 50, 2.7, 0.8],
    [0.73, -1, 46, 2.5, 0.2], [0.89, 1, 49, 2.9, 1.1]
  ], 2.6, 0.2, true);
  scatter("./lib/space-rock-a.glb", [
    [0.17, -1, 44, 2.2, 0.5], [0.39, 1, 46, 2.8, 1.3], [0.62, -1, 45, 2.4, 0.1], [0.86, 1, 43, 2.6, 0.9]
  ], 2.4, 0.38, true);
  scatter("./lib/cactus_tall.glb", [
    [0.14, -1, 28, 2.4, 0.2], [0.32, 1, 29, 2.8, 0.8], [0.58, -1, 27, 2.2, 1.4],
    [0.77, 1, 30, 2.6, 0.3], [0.94, -1, 28, 2.3, 1.0]
  ], 2.5, 0.55, 0.7);
  scatter("./lib/cactus_short.glb", [
    [0.10, 1, 26, 1.4, 0.4], [0.26, -1, 25, 1.6, 1.1], [0.49, 1, 27, 1.5, 0.2], [0.81, -1, 26, 1.3, 0.7]
  ], 1.4, 0.55, 0.45);
  scatter("./lib/desert-scrub.glb", [
    [0.08, 1, 24, 1.3, 0.1], [0.18, -1, 26, 1.5, 0.6], [0.29, 1, 25, 1.2, 1.2],
    [0.40, -1, 27, 1.4, 0.3], [0.51, 1, 24, 1.3, 0.9], [0.64, -1, 25, 1.6, 0.2],
    [0.75, 1, 26, 1.4, 1.0], [0.87, -1, 24, 1.2, 0.5], [0.97, 1, 27, 1.5, 0.8]
  ], 1.4, 0.25, false);
  scatter("./lib/dune-grass-tuft.glb", [
    [0.07, -1, 22, 1.1, 0.2], [0.22, 1, 23, 1.2, 0.7], [0.37, -1, 22, 1.0, 1.1],
    [0.53, 1, 24, 1.2, 0.4], [0.69, -1, 23, 1.1, 0.9], [0.85, 1, 22, 1.3, 0.2]
  ], 1.1, 0.5, false);
  scatter("./lib/oil-drum.glb", [
    [0.15, 1, 20, 1.35, 0.2], [0.34, -1, 21, 1.4, 0.8], [0.56, 1, 20, 1.3, 0.1], [0.82, -1, 21, 1.35, 1.2]
  ], 1.35, 0, 0.7);
  scatter("./lib/oil-drum-stack.glb", [
    [0.38, -1, 58, 2.4, 0.2], [0.72, 1, 31, 2.2, 0.6]
  ], 2.3, 0, 1.2);
  scatter("./lib/crate-01.glb", [
    [0.16, -1, 21, 1.1, 0.3], [0.42, 1, 22, 1.2, 0.9], [0.67, 1, 21, 1.15, 0.2], [0.90, -1, 22, 1.1, 1.0]
  ], 1.15, 0.15, 0.6);
  scatter("./lib/ammo-crate.glb", [
    [0.25, 1, 21, 1.0, 0.4], [0.71, -1, 22, 1.05, 0.8]
  ], 1.0, 0, 0.55);
  scatter("./lib/space-barrel.glb", [
    [0.33, 1, 23, 1.5, 0.2], [0.60, -1, 22, 1.4, 1.3]
  ], 1.45, 0.1, 0.7);
  scatter("./lib/debris-pile.glb", [
    [0.23, -1, 34, 1.8, 0.3], [0.47, 1, 33, 2.1, 0.9], [0.78, 1, 35, 1.9, 0.1]
  ], 1.9, 0.3, 0.9);
  scatter("./lib/crater.glb", [
    [0.12, -1, 70, 6.5, 0.2], [0.46, 1, 78, 8.0, 0.8], [0.83, -1, 74, 7.2, 1.4]
  ], 7, 0.4, false);
  scatter("./lib/craterLarge.glb", [
    [0.30, 1, 90, 11, 0.3], [0.68, -1, 88, 12, 1.1]
  ], 11, 0.4, false);

  loadGlb("./lib/building-a.glb", (root) => {
    const at = alongTrack(0.38, -1, 78);
    marsPaint(root, 0.28);
    sitGlb(root, at.x, at.z, 9.5, at.yaw + 0.15, { collider: 5.5, bury: 0.2, clear: 10, keepOffRocket: true });
  });
  loadGlb("./lib/detail-tank.glb", (tmpl) => {
    [[0.36, -1, 70, 4.2], [0.40, -1, 72, 3.6]].forEach((s) => {
      const at = alongTrack(s[0], s[1], s[2]);
      const r = cloneGlb(tmpl);
      marsPaint(r, 0.2);
      sitGlb(r, at.x, at.z, s[3], at.yaw, { collider: 2.2, bury: 0.1, clear: 8, keepOffRocket: true });
    });
  });
  loadGlb("./lib/chimney-medium.glb", (root) => {
    const at = alongTrack(0.37, -1, 82);
    marsPaint(root, 0.22);
    sitGlb(root, at.x, at.z, 7.5, at.yaw, { collider: 1.6, bury: 0.1, clear: 8, keepOffRocket: true });
  });
  loadGlb("./lib/machine.glb", (root) => {
    const at = alongTrack(0.39, -1, 60);
    marsPaint(root, 0.18);
    sitGlb(root, at.x, at.z, 2.8, at.yaw + 0.4, { collider: 1.4, bury: 0.05, clear: 6, keepOffRocket: true });
  });
  loadGlb("./lib/satelliteDish.glb", (tmpl) => {
    [[0.19, 1, 48], [0.45, 1, 52], [0.80, 1, 50]].forEach((s) => {
      const at = alongTrack(s[0], s[1], s[2]);
      if (nearRocket(at.x, at.z)) return;
      sitGlb(cloneGlb(tmpl), at.x, at.z, 3.4, at.yaw, { collider: 1.3, bury: 0.04, clear: 6, keepOffRocket: true });
    });
  });
  loadGlb("./lib/structure.glb", (root) => {
    const at = alongTrack(0.61, -1, 52);
    marsPaint(root, 0.2);
    sitGlb(root, at.x, at.z, 5.5, at.yaw, { collider: 3.2, bury: 0.12, clear: 8, keepOffRocket: true });
  });
  loadGlb("./lib/rover.glb", (root) => {
    const at = alongTrack(0.46, 1, 30);
    sitGlb(root, at.x, at.z, 2.2, at.yaw + 0.5, { collider: 1.6, bury: 0.02, clear: 6, keepOffRocket: true });
  });
  if (!nMobile) {
    loadGlb("./lib/obelisk.glb", (root) => {
      const at = alongTrack(0.55, 1, 62);
      marsPaint(root, 0.35);
      sitGlb(root, at.x, at.z, 6.5, at.yaw, { collider: 1.2, bury: 0.15, clear: 8, keepOffRocket: true });
    });
    loadGlb("./lib/date-palm.glb", (root) => {
      const at = alongTrack(0.042, 1, 22);
      if (nearRocket(at.x, at.z)) return;
      sitGlb(root, at.x, at.z, 4.8, at.yaw, { collider: 0.8, bury: 0.08, clear: 5, keepOffRocket: true });
    });
  }
}
function plantPeopleOffPad() {
  const workerMat = new THREE.MeshStandardMaterial({
    color: 0x4A5560, roughness: 0.62, metalness: 0.18, side: THREE.DoubleSide
  });
  const workerMatB = new THREE.MeshStandardMaterial({
    color: 0xB85828, roughness: 0.58, metalness: 0.12, side: THREE.DoubleSide
  });
  const womanMat = new THREE.MeshStandardMaterial({
    color: 0xC4B8A0, roughness: 0.55, metalness: 0.08, side: THREE.DoubleSide
  });
  const workerSpots = [
    [0.19, 1, 32, 2.38, 0.3], [0.31, -1, 34, 2.36, 2.4],
    [0.45, 1, 36, 2.40, -0.6], [0.61, -1, 32, 2.34, 1.1],
    [0.80, 1, 34, 2.38, 0.8]
  ];
  loadStl("./mesh/worker.stl", (geo) => {
    workerSpots.slice(0, IS_MOBILE ? 3 : workerSpots.length).forEach((s, i) => {
      const at = alongTrack(s[0], s[1], s[2]);
      if (nearRocket(at.x, at.z)) return;
      sitMesh(geo, at.x, at.z, s[3], i % 2 ? workerMatB : workerMat, at.yaw + s[4], { collider: 0.55, bury: 0.02, clear: 5 });
    });
  });
  if (!IS_MOBILE) {
    loadStl("./mesh/woman.stl", (geo) => {
      [[0.19, 1, 30, 2.30, 3.1], [0.45, 1, 34, 2.30, 0.4], [0.80, 1, 32, 2.30, 1.2]].forEach((s) => {
        const at = alongTrack(s[0], s[1], s[2]);
        if (nearRocket(at.x, at.z)) return;
        sitMesh(geo, at.x, at.z, s[3], womanMat, at.yaw + s[4], { collider: 0.5, bury: 0.02, clear: 5 });
      });
    });
  }
  const chars = [
    ["./lib/character-male-a.glb", 0.28, -1, 26, 2.15],
    ["./lib/character-female-a.glb", 0.41, 1, 28, 2.1],
    ["./lib/character-male-c.glb", 0.63, 1, 26, 2.15],
    ["./lib/character-female-c.glb", 0.88, 1, 25, 2.1]
  ];
  chars.forEach((s, i) => {
    if (IS_MOBILE && i > 1) return;
    loadGlb(s[0], (root) => {
      const at = alongTrack(s[1], s[2], s[3]);
      if (nearRocket(at.x, at.z)) return;
      sitGlb(root, at.x, at.z, s[4], at.yaw + (s[2] > 0 ? Math.PI : 0), { collider: 0.45, bury: 0.02, clear: 5, keepOffRocket: true });
    });
  });
}
...
function dustSprite() {
  ...
  grd.addColorStop(0, "rgba(240,140,70,1)");
  grd.addColorStop(0.4, "rgba(200,80,40,0.55)");
  grd.addColorStop(1, "rgba(160,50,24,0)");
  g.fillStyle = grd;
...
function seedHazards() {
  ...
  const crateMat = new THREE.MeshStandardMaterial({ color: 0x8B5A2B, roughness: 0.78, metalness: 0.08 });
  const drumMat = new THREE.MeshStandardMaterial({ color: 0xC45C18, roughness: 0.45, metalness: 0.35, emissive: 0x3a1808, emissiveIntensity: 0.25 });
  const airMat = new THREE.MeshStandardMaterial({ color: 0x7a2a18, roughness: 0.45, metalness: 0.25, emissive: 0xaa2200, emissiveIntensity: 0.55 });
  const warnMat = new THREE.MeshBasicMaterial({ color: 0xFFEE55, toneMapped: false, fog: false });
  for (let i = 0; i < DIFF.nGround; i++) {
    const t = 0.10 + (i + 0.5) / Math.max(1, DIFF.nGround) * 0.84;
    const side = i % 2 ? 1 : -1;
    const lat = side * (DIFF.lat0 + (i % 3) * ((DIFF.lat1 - DIFF.lat0) / 3));
    const at = placeOnLat(t, lat);
    const g = new THREE.Group();
    const drum = i % 2 === 0;
    const body = drum
      ? new THREE.Mesh(new THREE.CylinderGeometry(1.28, 1.38, 2.45, 12), drumMat)
      : new THREE.Mesh(new THREE.BoxGeometry(2.55, 2.35, 2.55), crateMat);
    body.position.y = drum ? 1.22 : 1.18;
    body.castShadow = true;
    const stripe = new THREE.Mesh(new THREE.BoxGeometry(2.7, 0.14, 0.32), warnMat);
    ...
    g.frustumCulled = false;
    scene.add(g);
    hazards.push({ x: at.x, z: at.z, y: terrainH(at.x, at.z) + 1.2, r: 2.15, air: false, mesh: g, coolUntil: 0 });
  }
  const jumpAir = JUMPS.filter((j) => j.kind === "valley" || j.kind === "table");
  for (let i = 0; i < DIFF.nAir; i++) {
    const j = jumpAir[i % jumpAir.length];
    const u = i < jumpAir.length ? 0.28 : 0.40;
    const t = (j.t + u * j.len / trackLen) % 1;
    const y = terrainH(placeOnLat(t, 0).x, placeOnLat(t, 0).z) + 6.2;
    const g = new THREE.Group();
    const body = new THREE.Mesh(new THREE.SphereGeometry(1.25, 10, 8), airMat);
    const halo = new THREE.Mesh(new THREE.TorusGeometry(1.85, 0.1, 6, 14), warnMat);
    halo.rotation.x = Math.PI / 2;
    g.add(body, halo);
    g.frustumCulled = false;
    scene.add(g);
    hazards.push({
      t, y, r: 1.65, air: true, moving: true, amp: HALF_W * 0.72,
      phase: i * 1.15, speed: DIFF.airHz, mesh: g, coolUntil: 0, x: 0, z: 0
    });
  }
  stepHazards(0);
}
function stepHazards(dt) {
  for (let i = 0; i < hazards.length; i++) {
    ...
    h.x = at.x; h.z = at.z;
    if (h.mesh) {
      h.mesh.position.set(at.x, h.y, at.z);
...
function hitHazards(spdAbs, groundY) {
  for (let i = 0; i < hazards.length; i++) {
    ...
    if (d > h.r + 1.35) continue;
    if (h.air) {
      if (Math.abs(car.y - h.y) > 2.6) continue;
    } else if (car.air && car.y > groundY + 2.4) continue;
    h.coolUntil = timeMs + 650;
    const nx = (car.x - h.x) / (d || 1), nz = (car.z - h.z) / (d || 1);
    car.x += nx * 0.55; car.z += nz * 0.55;
    car.speed *= DIFF.slow;
    if (car.air) car.vy *= 0.45;
    if (DIFF.spin && spdAbs > 16 && car.spinT <= 0) spinOut(0.5);
    else showToast("CRASH", 0.4);
    emitDust(car.x, car.y + 0.4, car.z, nx, nz, 10);
  }
}
function seedCoins() {
  while (coins.length) {
  ...
  const mat = new THREE.MeshBasicMaterial({ color: 0xFFD700, toneMapped: false, fog: false });
  for (let i = 0; i < 16; i++) {
    const t = 0.065 + i * 0.054;
    const lat = (i % 2 ? 1 : -1) * 4.4;
    const at = placeOnLat(t, lat);
    const mesh = new THREE.Mesh(new THREE.CylinderGeometry(0.62, 0.62, 0.12, 16), mat);
    mesh.rotation.x = Math.PI / 2;
    mesh.position.set(at.x, terrainH(at.x, at.z) + 1.4, at.z);
    mesh.frustumCulled = false;
    scene.add(mesh);
    coins.push({ x: at.x, z: at.z, mesh, taken: false });
  }
}
function stepCoins(dt) {
  for (let i = 0; i < coins.length; i++) {
    ...
    if (c.mesh && !c.taken) {
      ...
      c.mesh.position.y = terrainH(c.x, c.z) + 1.4 + Math.sin(timeMs * 0.007 + i) * 0.18;
    }
    if (c.taken || startLock || finished) continue;
    if (Math.hypot(car.x - c.x, car.z - c.z) > 2.1) continue;
    c.taken = true;
    if (c.mesh) c.mesh.visible = false;
    coinsGot += 1;
    showToast("+" + coinsGot + "  COIN", 0.35);
  }
}
function raceScore() {
  const timePts = Math.max(0, Math.floor((200000 - timeMs) / 20));
  return coinsGot * 1000 + timePts;
}
const HS_KEY = "rdb-hs-v1";
function loadHs() {
  try {
...
function saveHs(list) {
  try { localStorage.setItem(HS_KEY, JSON.stringify(list.slice(0, 3))); } catch (err) {}
}
function renderHsBoard() {
  ...
  el.innerHTML = list.length
    ? list.map((r, i) => "<li><b>" + (r.name || "----") + "</b>  " + r.score + "  <span>" + (r.diff || "") + " · " + (r.coins || 0) + "c · " + fmtTime(r.time || 0) + "</span></li>").join("")
    : "<li>NO SCORES YET</li>";
}
function showFinish(label) {
  ...
  if (elScore) elScore.textContent = raceScore() + " PTS   ·   " + coinsGot + " COINS";
...
function submitHs() {
  if (!finished) return;
  if (hsSubmitted) { showToast("ALREADY SAVED", 0.45); return; }
  const inp = document.getElementById("hs-name");
  const name = commitName((inp && inp.value) || playerName);
  const row = { name, score: raceScore(), coins: coinsGot, time: timeMs, diff: DIFF.tag };
  const list = loadHs().concat(row).sort((a, b) => b.score - a.score).slice(0, 3);
  saveHs(list);
  hsSubmitted = true;
  renderHsBoard();
  showToast("SAVED  " + name, 0.7);
}
...
async function shareChallenge() {
  ...
  const url = location.href.split("?")[0] + "?d=" + DIFF.key + (g ? "&g=" + g : "");
  const text = playerName + "  Red Dust Baja " + DIFF.tag + "  " + fmtTime(timeMs) + " — beat my ghost";
  try {
    if (navigator.share) {
      await navigator.share({ title: "Red Dust Baja", text, url });
      showToast("SENT", 0.7);
      return;
    }
  } catch (err) {}
...
document.querySelectorAll(".boot-btn").forEach((btn) => {
...
(function seedBootPaint() {
  ...
  if (row) {
    ...
    LIVERIES.forEach((L, i) => {
      ...
      b.addEventListener("click", (e) => { e.preventDefault(); e.stopPropagation(); applyLivery(i, true); });
      row.appendChild(b);
    });
  }
...
function drive(dt) {
  ...
  prevDriveT = lastT;
  lastT = proj.t;
  if (!finished && timeMs - lastGhostMs > 80) {
    lastGhostMs = timeMs;
...
function grant(kind) {
  ...
  boostChain += 1;
  if (kind === "PAD") {
    car.boost = Math.min(1, car.boost + 0.42);
    car.turboT = Math.max(car.turboT, 0.75);
    showToast(boostChain > 1 ? "PAD  x" + boostChain : "PAD", 0.45);
  } else if (kind === "RING") {
    car.boost = Math.min(1, car.boost + 0.55);
    car.turboT = Math.max(car.turboT, 1.35);
    if (car.air) car.vy += 2.2;
    showToast(boostChain > 1 ? "AIR  x" + boostChain : "AIR", 0.5);
  } else {
    car.turboT = Math.max(car.turboT, 1.7);
    car.boost = Math.min(1, car.boost + 0.28);
    showToast(boostChain > 1 ? "TURBO  x" + boostChain : "TURBO", 0.6);
  }
}
function addPickup(kind, t, lat, yOff, cool) {
  const p = curve.getPointAt(t);
  const tan = curve.getTangentAt(t);
  const right = new THREE.Vector3(tan.z, 0, -tan.x).normalize();
  ...
  if (kind === "PAD") {
    ...
    const plate = new THREE.Mesh(new THREE.BoxGeometry(3.6, 0.16, 4.4), mat);
    plate.position.y = 0;
    mesh.add(plate);
    const arrow = new THREE.Mesh(new THREE.ConeGeometry(0.9, 1.6, 3), mat);
    arrow.rotation.x = Math.PI / 2;
    arrow.position.set(0, 0.55, 0.4);
    mesh.add(arrow);
  } else if (kind === "RING") {
    const mat = new THREE.MeshBasicMaterial({ color: 0xFF7A22, toneMapped: false, fog: false });
    const ring = new THREE.Mesh(new THREE.TorusGeometry(3.3, 0.24, 8, 22), mat);
    mesh.add(ring);
    ...
    const halo = new THREE.Mesh(
      new THREE.TorusGeometry(1.55, 0.12, 8, 20),
      new THREE.MeshBasicMaterial({ color: 0xFFEE88, toneMapped: false, fog: false })
    );
    halo.rotation.x = Math.PI / 2;
    mesh.add(core);
  ...
  mesh.frustumCulled = false;
  scene.add(mesh);
  pickups.push({ kind, x, z, y, mesh, taken: 0, cool: cool || 8, hitR: kind === "RING" ? 5.4 : 4.2 });
}
(function seedPickups() {
  const pads = [0.08, 0.17, 0.27, 0.36, 0.47, 0.61, 0.71, 0.81, 0.91];
  pads.forEach((t, i) => addPickup("PAD", t, (i % 2 ? 1 : -1) * 3.6, 0.55, 6.5));
  addPickup("TURBO", 0.15, 17.5, 2.1, 11);
  addPickup("TURBO", 0.33, -17.8, 2.1, 11);
  addPickup("TURBO", 0.40, 18.2, 2.1, 11);
  addPickup("TURBO", 0.66, 17.6, 2.1, 11);
  addPickup("TURBO", 0.80, -18.0, 2.1, 11);
  JUMPS.forEach((j) => {
    if (j.kind !== "valley" && j.kind !== "table") return;
    const lip = (j.t + 0.24 * j.len / trackLen) % 1;
    const air = (j.t + 0.40 * j.len / trackLen) % 1;
    addPickup("RING", lip, 0, 5.6, 8);
    addPickup("RING", air, 0, j.kind === "valley" ? 9.2 : 6.4, 8);
  });
})();
function stepRivals(dt) {
  ...
  pickups.forEach((p) => {
    if (p.mesh) {
      p.mesh.rotation.y += dt * (p.kind === "RING" ? 1.5 : 2.4);
      const base = p.y != null ? p.y : terrainH(p.x, p.z) + 2.1;
      p.mesh.position.y = base + Math.sin(timeMs * 0.006 + p.x) * (p.kind === "PAD" ? 0.06 : 0.32);
      p.mesh.scale.setScalar(1 + Math.sin(timeMs * 0.008) * 0.08);
    }
  ...
  if (!finished && r.laps >= LAP_COUNT && r.t > START_T + 0.01) {
    finished = true;
    playerWon = false;
...
function hudBind() {
  ...
  elGateIdx.textContent = "L" + Math.min(LAP_COUNT, raceLap + 1) + "/" + LAP_COUNT + "  " + (finished ? GATE_COUNT : nextGate + 1) + "/" + GATE_COUNT;
  ...
  if (elCoinVal) elCoinVal.textContent = String(coinsGot);
  if (elCount) {
...
if (waitingDiff) {
  ...
  if (ghostIn && elBootSub) elBootSub.textContent = "Ghost challenge loaded. Pick a line and beat their run.";
} else {
  seedHazards();
  seedCoins();
  if (elBoot) elBoot.classList.remove("show");
