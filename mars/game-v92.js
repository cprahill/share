...
// cache: game-v92.js  — production dress, corridor, coins, turbos on dirt
const BUILD = "92";
...
function packedTrackTexture() {
  ...
  for (let i = 0; i < 18000; i++) {
    ...
    g.fillStyle = "rgba(" + (130 + v) + "," + (78 + (v * 0.55) | 0) + "," + (48 + (v * 0.3) | 0) + "," + (0.14 + Math.random() * 0.32) + ")";
...
function sitMesh(geo, x, z, worldH, mat, rotY, opts) {
  ...
  const clear = opts.clear != null ? opts.clear : 12;
  if (!opts.allowPath && minDistToPath(x, z) - foot * 0.5 < HALF_W + clear) return null;
...
(function worldDress() {
  ...
  {
    const gantSlots = IS_MOBILE
      ? [[0.50, -1, 40], [0.52, 1, 42], [0.55, -1, 40], [0.57, 1, 41]]
      : [[0.49, -1, 42], [0.505, 1, 44], [0.52, -1, 42], [0.535, 1, 44], [0.55, -1, 42], [0.565, 1, 43]];
    gantSlots.forEach((s) => {
      const at = alongTrack(s[0], s[1], s[2]);
      registerShipFire(at.x, at.z, 52);
    });
  }
  ...
  loadStl("./mesh/obj_3_Ship.stl", (geo) => {
    ...
    const gant = IS_MOBILE
      ? [[0.50, -1, 40, 40, 0.1], [0.52, 1, 42, 44, 0.4], [0.55, -1, 40, 42, 1.1], [0.57, 1, 41, 41, 0.2]]
      : [
        [0.49, -1, 42, 42, 0.1], [0.505, 1, 44, 46, 0.4],
        [0.52, -1, 42, 40, 1.1], [0.535, 1, 44, 48, 0.2],
        [0.55, -1, 42, 44, 0.7], [0.565, 1, 43, 41, 1.4]
      ];
    gant.forEach((s, i) => {
      const at = alongTrack(s[0], s[1], s[2]);
...
function dressColony() {
  ...
  put("./lib/platform_long.glb", 0.14, -1, 48, 1.4, 0, { collider: 3.2, bury: 0.04 });
  ...
  put("./lib/hangar_roundGlass.glb", 0.62, 1, 52, 14, 0, { collider: 6.5 });
  put("./lib/hangar_roundA.glb", 0.66, -1, 54, 12, 0.2, { collider: 5.8 });
...
function plantPeopleOffPad() {
  ...
  const workerSpots = [
    [0.14, -1, 46, 2.38, 0.3], [0.28, -1, 44, 2.36, 2.4],
    [0.41, 1, 48, 2.40, -0.6], [0.64, 1, 46, 2.34, 1.1],
    [0.88, 1, 46, 2.38, 0.8]
  ];
  loadStl("./mesh/worker.stl", (geo) => {
    workerSpots.slice(0, IS_MOBILE ? 3 : workerSpots.length).forEach((s, i) => {
      ...
      sitMesh(geo, p.x, p.z, s[3], i % 2 ? workerMatB : workerMat, p.yaw + s[4], { collider: 0.55, bury: 0.02, clear: 12 });
    });
  });
  if (!IS_MOBILE) {
    loadStl("./mesh/woman.stl", (geo) => {
      [[0.14, -1, 44, 2.30, 3.1], [0.41, 1, 46, 2.30, 0.4], [0.88, 1, 44, 2.30, 1.2]].forEach((s) => {
        const p = alongTrack(s[0], s[1], s[2]);
        if (nearShipFire(p.x, p.z)) return;
        sitMesh(geo, p.x, p.z, s[3], womanMat, p.yaw + s[4], { collider: 0.5, bury: 0.02, clear: 12 });
      });
    });
  }
  const chars = [
    ["./lib/character-male-a.glb", 0.14, -1, 46, 2.15],
    ["./lib/character-female-a.glb", 0.41, 1, 48, 2.1],
    ["./lib/character-male-c.glb", 0.64, 1, 46, 2.15],
    ["./lib/character-female-c.glb", 0.96, -1, 48, 2.1]
  ];
  chars.forEach((s, i) => {
    if (IS_MOBILE && i > 1) return;
    loadGlb(s[0], (root) => {
      const p = alongTrack(s[1], s[2], s[3]);
      if (nearShipFire(p.x, p.z)) return;
      sitGlb(root, p.x, p.z, s[4], p.yaw + (s[2] > 0 ? Math.PI : 0), { collider: 0.45, bury: 0.02, clear: 12, keepOffRocket: true });
    });
  });
}
...
function seedCoins() {
  ...
  const gold = new THREE.MeshStandardMaterial({
    color: 0xFFD24A, metalness: 0.65, roughness: 0.22,
    emissive: 0xFFEE66, emissiveIntensity: 1.35, toneMapped: false, fog: false
  });
  const haloMat = new THREE.MeshBasicMaterial({ color: 0x3de8ff, toneMapped: false, fog: false });
  const rimMat = new THREE.MeshBasicMaterial({ color: 0xFFF6C0, toneMapped: false, fog: false });
  for (let i = 0; i < 16; i++) {
    const t = 0.065 + i * 0.054;
    const lat = (i % 2 ? 1 : -1) * 5.2;
    const at = placeOnLat(t, lat);
    const mesh = new THREE.Group();
    const core = new THREE.Mesh(new THREE.CylinderGeometry(1.15, 1.15, 0.22, 22), gold);
    core.rotation.x = Math.PI / 2;
    const halo = new THREE.Mesh(new THREE.TorusGeometry(1.42, 0.08, 8, 24), haloMat);
    const rim = new THREE.Mesh(new THREE.TorusGeometry(1.12, 0.05, 6, 20), rimMat);
    mesh.add(core, halo, rim);
    mesh.position.set(at.x, terrainH(at.x, at.z) + 2.6, at.z);
    mesh.frustumCulled = false;
    scene.add(mesh);
...
function stepCoins(dt) {
  for (let i = 0; i < coins.length; i++) {
    ...
    if (c.mesh && !c.taken) {
      ...
      c.mesh.position.y = terrainH(c.x, c.z) + 2.6 + Math.sin(timeMs * 0.007 + i) * 0.28;
    }
    if (c.taken || startLock || finished) continue;
    if (Math.hypot(car.x - c.x, car.z - c.z) > 2.8) continue;
    c.taken = true;
...
(function seedPickups() {
  ...
  addPickup("TURBO", 0.15, 8.0, 2.1, 11);
  addPickup("TURBO", 0.33, -8.0, 2.1, 11);
  addPickup("TURBO", 0.40, 8.0, 2.1, 11);
  addPickup("TURBO", 0.66, 8.0, 2.1, 11);
  addPickup("TURBO", 0.80, -8.0, 2.1, 11);
  JUMPS.forEach((j) => {
