...
// cache: game-v93.js  — floating BOOST, more coins, geodesic biodomes
const BUILD = "93";
...
function sitMesh(geo, x, z, worldH, mat, rotY, opts) {
  ...
  const clear = opts.clear != null ? opts.clear : 14;
...
function seedCoins() {
  ...
  for (let i = 0; i < 28; i++) {
    const t = 0.055 + i * 0.032;
    if (t > 0.47 && t < 0.59) continue;
    const lat = (i % 3 === 0 ? 0 : (i % 3 === 1 ? 1 : -1) * 4.2);
    const at = placeOnLat(t, lat);
...
function grant(kind) {
  ...
  if (kind === "RING") {
    ...
    car.turboT = Math.max(car.turboT, 1.6);
    car.boost = Math.min(1, car.boost + 0.5);
    showToast(boostChain > 1 ? "BOOST  x" + boostChain : "BOOST", 0.55);
  }
}
function addPickup(kind, t, lat, yOff, cool) {
  ...
  if (kind === "RING") {
    ...
    const halo = new THREE.Mesh(
      new THREE.TorusGeometry(1.7, 0.14, 8, 22),
  ...
  mesh.frustumCulled = false;
  scene.add(mesh);
  pickups.push({ kind, x, z, y, mesh, taken: 0, cool: cool || 8, hitR: kind === "RING" ? 5.4 : 3.8 });
}
(function seedPickups() {
  const boosts = [
    [0.18, 0], [0.28, 0], [0.36, 0], [0.48, 0], [0.60, 0], [0.82, 0]
  ];
  boosts.forEach((s) => addPickup("BOOST", s[0], s[1], 2.5, 9));
  JUMPS.forEach((j) => {
