...
(function (w) {
  ...
  G.paddle = (api) => (ctx, dt, io) => {
    ...
    if (st.x * W < 24 && st.y * H > ly && st.y * H < ly + ph) { st.vx = Math.abs(st.vx); api.sfx.hit && api.sfx.hit(); }
    if (st.x * W > W - 24 && st.y * H > ry && st.y * H < ry + ph) { st.vx = -Math.abs(st.vx); api.sfx.hit && api.sfx.hit(); }
    if (st.x < 0) { st.rs++; st.x = 0.5; st.vx = 0.35; api.sfx.die && api.sfx.die(); }
    if (st.x > 1) { st.ls++; st.x = 0.5; st.vx = -0.35; api.sfx.die && api.sfx.die(); }
    api.hud(st.ls + "  —  " + st.rs);
    if (st.ls >= 11 || st.rs >= 11) {
      st.dead = true;
      api.over(st.ls >= 11 ? "LEFT WINS" : "RIGHT WINS", st.ls + "–" + st.rs);
    }
    ctx.fillStyle = "#000";
    ctx.fillRect(0, 0, W, H);
    ctx.fillStyle = "#cfc";
    for (let y = 0; y < H; y += 16) ctx.fillRect(W / 2 - 2, y, 4, 8);
    ctx.fillRect(8, ly, pw, ph);
    ctx.fillRect(W - 18, ry, pw, ph);
    ctx.beginPath();
    ctx.arc(st.x * W, st.y * H, 6, 0, 7);
    ctx.fill();
  };
  ...
  G.bricks = (api) => (ctx, dt, io) => {
    ...
    api.hud(s.score + "  ·  " + s.lives);
    ctx.fillStyle = "#08060c";
    ctx.fillRect(0, 0, W, H);
    const pal = ["#e44", "#e84", "#ee4", "#4e8", "#48e"];
    const bw = (W - 24) / cols, bh = 16;
    for (const b of s.bricks) {
      if (!b.on) continue;
      ctx.fillStyle = pal[b.r];
      ctx.fillRect(12 + b.c * bw, 64 + b.r * (bh + 4), bw - 4, bh);
    }
    ctx.fillStyle = "#eee";
  ...
  G.night = (api) => (ctx, dt, io) => {
    ...
    for (const c of s.cars) {
      ...
      ctx.fillStyle = "#c33";
      ctx.fillRect(x - sc / 2, y - sc * 0.4, sc, sc * 0.55);
    }
    ctx.fillStyle = "#4ae";
    ctx.fillRect(s.x * W - 16, H - 48, 32, 22);
  };
  ...
  G.armor = (api) => (ctx, dt, io) => {
    ...
    for (const w of s.walls) ctx.fillRect(w.x * W, w.y * H, w.w * W, w.h * H);
    function tank(t, col) {
      ctx.save();
    ...
    tank(s.b, "#da4");
    ctx.fillStyle = "#fff";
    for (const sh of s.shots) ctx.fillRect(sh.x * W - 2, sh.y * H - 2, 4, 4);
  };
  ...
  G.rows = (api) => (ctx, dt, io) => {
    ...
    api.hud(st.score + "  ·  " + st.lives);
    ctx.fillStyle = "#000";
    ctx.fillRect(0, 0, W, H);
    ctx.fillStyle = "#3e3";
    for (const a of st.aliens) {
      if (!a.on) continue;
      ctx.fillRect((0.12 + a.c * 0.1) * W - 10, (0.18 + a.r * 0.07) * H, 20, 12);
    }
    ctx.fillStyle = "#4a4";
    for (const b of st.bunk) if (b.hp > 0) ctx.fillRect(b.x * W - 18, H * 0.78, 36, 14);
    ctx.fillStyle = "#eee";
    ctx.fillRect(st.px * W - 14, H * 0.9, 28, 10);
    ctx.fillStyle = "#fff";
  ...
  G.rocks = (api) => (ctx, dt, io) => {
    ...
    ctx.fillStyle = "#fff";
    for (const sh of s.shots) ctx.fillRect(sh.x * W - 1, sh.y * H - 1, 3, 3);
  };
  ...
  G.hopper = (api) => (ctx, dt, io) => {
    ...
    const lh = (H - 48) / 9;
    for (let i = 0; i < 9; i++) {
      ctx.fillStyle = i === 0 || i === 8 ? "#163" : (i % 2 ? "#333" : "#222");
      ctx.fillRect(0, 48 + i * lh, W, lh);
    }
    ctx.fillStyle = "#c33";
    for (const c of s.cars) ctx.fillRect(c.x * W - 18, 48 + c.lane * lh + 6, 36, lh - 12);
    ctx.fillStyle = "#6e6";
    ctx.fillRect(s.x * W - 10, 48 + fy * lh + 8, 20, lh - 16);
  };
  ...
  G.bugs = (api) => (ctx, dt, io) => {
    ...
    if (s.sh) {
      ...
      if (hit >= 0) { s.segs.splice(hit, 1); s.sh = null; s.score += 25; api.sfx.hit && api.sfx.hit(); }
    }
    ...
    ctx.fillRect(0, 0, W, H);
    ctx.fillStyle = "#5a3";
    for (const m of s.mush) ctx.fillRect(m.x * W - 6, m.y * H - 6, 12, 12);
    ctx.fillStyle = "#e44";
    for (const g of s.segs) ctx.fillRect(g.x * W - 8, g.y * H - 8, 16, 16);
    ctx.fillStyle = "#eee";
    ctx.fillRect(s.px * W - 12, H * 0.9, 24, 10);
    if (s.sh) ctx.fillRect(s.sh.x * W - 1, s.sh.y * H, 3, 10);
  };
  ...
  G.manor = (api) => (ctx, dt, io) => {
    ...
    if (s.x === s.gx && s.y === s.gy) { s.dead = true; api.over("CAUGHT", "SCORE " + s.score); api.sfx.die && api.sfx.die(); }
    ...
    for (let y = 0; y < 9; y++) for (let x = 0; x < 17; x++) {
      ...
      if (ch === "#") {
        ctx.fillStyle = "#226";
        ctx.fillRect(x * tw, 48 + y * th, tw - 1, th - 1);
    ...
    ctx.fillStyle = "#ff0";
    ctx.beginPath();
    ctx.arc(s.x * tw + tw / 2, 48 + s.y * th + th / 2, tw * 0.35, 0.4, 5.8);
    ctx.lineTo(s.x * tw + tw / 2, 48 + s.y * th + th / 2);
    ctx.fill();
    ctx.fillStyle = "#f36";
    ctx.beginPath();
    ctx.arc(s.gx * tw + tw / 2, 48 + s.gy * th + th / 2, tw * 0.35, 0, 7);
    ctx.fill();
  };
  ...
  G.swarm = (api) => (ctx, dt, io) => {
    ...
    ctx.fillRect(0, 0, W, H);
    ctx.fillStyle = "#fc4";
    for (const e of s.ships) ctx.fillRect(e.x * W - 12, e.y * H, 24, 14);
    ctx.fillStyle = "#8cf";
    ctx.fillRect(s.px * W - 14, H * 0.9, 28, 12);
    ctx.fillStyle = "#fff";
  ...
  G.rig = (api) => (ctx, dt, io) => {
    ...
    ctx.fillRect(0, 0, W, H);
    ctx.fillStyle = "#a62";
    for (const f of floors) ctx.fillRect(W * 0.06, f * H, W * 0.88, 8);
    ctx.fillStyle = "#888";
    for (const f of floors) ctx.fillRect(W * 0.9, f * H - 40, 8, 40);
    ctx.fillStyle = "#fa4";
    ctx.fillRect(s.x * W - 8, y * H - 22, 16, 22);
    ctx.fillStyle = "#c40";
    for (const b of s.barrels) {
      ctx.beginPath();
      ctx.arc(b.x * W, floors[b.fi] * H - 8, 8, 0, 7);
      ctx.fill();
    }
  };
  ...
  G.grid = (api) => (ctx, dt, io) => {
    ...
    for (const m of s.missiles) {
      for (const b of s.bursts) {
        if (Math.hypot(m.x - b.x, m.y - b.y) < b.r) { m.y = 2; s.score += 25; api.sfx.hit && api.sfx.hit(); }
      }
      if (m.y > 0.9) {
    ...
    ctx.fillStyle = "#8cf";
    for (const c of s.cities) if (c.on) ctx.fillRect(c.x * W - 12, H * 0.88, 24, 16);
    ctx.strokeStyle = "#f84";
    for (const m of s.missiles) {
      ctx.beginPath();
      ctx.moveTo(m.tx * W, 48);
  ...
  G.stack = (api) => (ctx, dt, io) => {
    ...
    const s = G.stack.s || (G.stack.s = {
      ...
      p: PIECES.T, x: 3, y: 0, t: 0, score: 0, dead: false, k: 0
    });
    if (s.dead) return;
    ...
    function spawn() {
      ...
      s.p = PIECES[names[(Math.random() * names.length) | 0]].map((r) => r.slice());
      s.x = 3; s.y = 0;
    ...
    if (s.t > grav) {
      ...
      if (!collide(s.p, s.x, s.y + 1)) s.y++;
        ...
        for (let r = 0; r < s.p.length; r++) for (let c = 0; c < s.p[r].length; c++) {
          if (s.p[r][c] && s.y + r >= 0) s.grid[s.y + r][s.x + c] = 1;
        }
    ...
    ctx.fillRect(0, 0, W, H);
    ctx.fillStyle = "#222";
    ctx.fillRect(ox - 4, oy - 4, cw * COLS + 8, ch * ROWS + 8);
    ctx.fillStyle = "#6cf";
    for (let r = 0; r < ROWS; r++) for (let c = 0; c < COLS; c++) if (s.grid[r][c]) ctx.fillRect(ox + c * cw, oy + r * ch, cw - 1, ch - 1);
    ctx.fillStyle = "#fc4";
    for (let r = 0; r < s.p.length; r++) for (let c = 0; c < s.p[r].length; c++) if (s.p[r][c]) ctx.fillRect(ox + (s.x + c) * cw, oy + (s.y + r) * ch, cw - 1, ch - 1);
  };
  ...
  G.foam = (api) => (ctx, dt, io) => {
    ...
    ctx.fillStyle = "#fc8";
    ctx.fillRect(s.x * W - 10, s.y * H - 20, 20, 24);
    ctx.fillStyle = "#adf";
    for (const f of s.foams) { ctx.beginPath(); ctx.arc(f.x * W, f.y * H, 10, 0, 7); ctx.fill(); }
    ctx.fillStyle = "#f46";
    for (const e of s.foes) ctx.fillRect(e.x * W - 10, e.y * H - 18, 20, 22);
  };
  ...
  G.stripe = (api) => (ctx, dt, io) => {
    ...
    ctx.fillStyle = "#000";
    for (const g of s.gaps) ctx.fillRect(g.x, H * 0.72, g.w, 14);
    ctx.fillStyle = "#4cf";
  ...
  G.ring = (api) => (ctx, dt, io) => {
    ...
    if (s.atk > 0 && Math.abs(s.x + 0.08 - s.e) < 0.08) { s.eh -= dt * 8; api.sfx.hit && api.sfx.hit(); }
    ...
    ctx.fillStyle = "#48f";
    ctx.fillRect(s.x * W - 14, H * 0.55, 28, 70);
    ctx.fillStyle = "#f44";
    ctx.fillRect(s.e * W - 14, H * 0.55, 28, 70);
    if (s.atk > 0) { ctx.fillStyle = "#fff"; ctx.fillRect(s.x * W + 14, H * 0.6, 22, 8); }
  };
  ...
  G.dual = (api) => (ctx, dt, io) => {
    ...
    for (const e of s.en) {
      for (const sh of s.sh) if (Math.hypot(e.x - sh.x, e.y - sh.y) < 0.03) { e.x = -1; s.score += 15; }
    ...
    ctx.fillStyle = "#f8f";
    for (const e of s.en) { ctx.beginPath(); ctx.arc(e.x * W, e.y * H, 7, 0, 7); ctx.fill(); }
    ctx.fillStyle = "#fff";
    for (const sh of s.sh) ctx.fillRect(sh.x * W - 1, sh.y * H - 1, 3, 3);
  };
  ...
  G.beat = (api) => (ctx, dt, io) => {
    ...
    for (let i = 0; i < 4; i++) {
      if (keys[lanes[i]]) {
        ...
        if (n) { n.y = 2; s.score += 10; api.sfx.pickup && api.sfx.pickup(); }
        else s.miss++;
      }
    }
    ...
    const lw = W / 4;
    ctx.fillStyle = "#fff4";
    ctx.fillRect(0, hitY * H - 6, W, 12);
    const col = ["#f66", "#6cf", "#6f6", "#fc6"];
    for (let i = 0; i < 4; i++) {
      ctx.fillStyle = col[i];
      ctx.globalAlpha = 0.2;
      ctx.fillRect(i * lw, 48, lw - 4, H);
      ctx.globalAlpha = 1;
    }
    for (const n of s.notes) {
      ctx.fillStyle = col[n.lane];
      ctx.fillRect(n.lane * lw + 8, n.y * H, lw - 20, 16);
    }
  };
  ...
  G.volt = (api) => (ctx, dt, io) => {
    ...
    ctx.fillRect(0, H * 0.72, W, H);
    ctx.fillStyle = "#fd4";
    for (const o of s.obs) ctx.fillRect(o.x, H * 0.72 - o.h, 18, o.h);
    ctx.fillStyle = "#6ef";
    ctx.fillRect(px, py - 24, 18, 24);
  };
